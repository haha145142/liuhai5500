...
export default async function handler(req:Request,res:Response){if(req.method!=="GET"&&req.method!=="POST")return res.status(405).json({ok:false,error:"method-not-allowed"});if(!auth(req))return res.status(401).json({ok:false,error:"unauthorized"});
  const configured=String(process.env.VAPID_PUBLIC_KEY||"").trim()&&String(process.env.VAPID_PRIVATE_KEY||"").trim();if(!configured)return res.status(503).json({ok:false,error:"vapid-not-configured"});
  ...
  for(const code of codes.slice(0,200)){
    ...
    for(const alert of alerts){const dedupe=await sharedCacheGet<boolean>(`fund-ai-pro:push:sent:${alert.key}`);if(dedupe?.value===true)continue;for(const sub of subscriptions){try{await sendWebPush(sub,{title:alert.title,body:alert.body,icon:"/icon-192.png",badge:"/icon-192.png",data:{url:alert.url}});sent.push(`${code}:${alert.key}`);}catch(error){const status=(error as Error&{status?:number}).status;if(status===404||status===410)expired.push(sub.endpoint);else failed.push(`${code}:${alert.key}`);}}
      await sharedCacheSet(`fund-ai-pro:push:sent:${alert.key}`,true,36*60*60_000);
    }
  }
