...
async function testNews(page, label) {
  await page.goto(`${baseURL}/news`, { waitUntil: "domcontentloaded" }); await waitForText(page, "市场资讯", `${label} news page`, 15_000); await waitForText(page, "AI解读", `${label} news AI interpretation`, 15_000);
  await page.waitForFunction(() => { const count = document.querySelectorAll(".news-page article").length; const body = document.body.innerText; return count > 0 || body.includes("暂无可靠资讯") || body.includes("正在抓取资讯"); }, { timeout: 15_000 });
  const body = await page.locator("body").innerText(); if (body.includes("查看原文")) throw new Error(`${label}: news page still renders original-article action`); if (body.includes("华尔街见闻")) throw new Error(`${label}: removed news source is still rendered`);
  const articleCount = await page.locator(".news-page article").count(); if (articleCount > 5) throw new Error(`${label}: news page renders ${articleCount} articles before expand; expected max 5`);
  if (articleCount > 0 && (await page.locator('button').filter({ hasText: /展开更多资讯/ }).count())) { await waitForText(page, "展开更多资讯", `${label} news collapse control`, 5_000); }
  await assertNoHorizontalOverflow(page, `${label} news page`);
}
...
try {
  ...
  for (const viewport of [{ width: 390, height: 844, label: "390x844" }, { width: 430, height: 932, label: "430x932" }]) {
    ...
    try {
      ...
      await page.goto(`${baseURL}/`, { waitUntil: "domcontentloaded" }); await waitForText(page, "总收益", `${viewport.label} homepage`); await waitForHydration(page, `${viewport.label} homepage`); await waitForText(page, "今日评估", `${viewport.label} daily assessment`); await waitForText(page, "今日市场评分", `${viewport.label} daily score`); if ((await page.locator("body").innerText()).trim().length < 80) throw new Error(`${viewport.label}: homepage rendered blank/minimal content`); await assertNoHorizontalOverflow(page, `${viewport.label} homepage`);
      await testNews(page, viewport.label); await testSectorFundPool(page, viewport.label); await testFundPk(page, viewport.label); await testIndexValuation(page, viewport.label); await testSmartAlerts(page, viewport.label); await testRotationRadar(page, viewport.label);
