...
async function testIndexValuation(page, label) {
  await page.goto(`${baseURL}/market`, { waitUntil: "domcontentloaded" }); await waitForText(page, "指数估值红绿灯", `${label} index valuation`); await waitForText(page, "PE", `${label} PE metric`); await waitForText(page, "PB", `${label} PB metric`); await waitForText(page, "ROE", `${label} ROE metric`); await waitForText(page, "暂无可靠数据", `${label} unavailable valuation state`, 15_000); await assertNoHorizontalOverflow(page, `${label} index valuation`);
}
...
try {
  ...
  for (const viewport of [{ width: 390, height: 844, label: "390x844" }, { width: 430, height: 932, label: "430x932" }]) {
    ...
    try {
      ...
      await testSectorFundPool(page, viewport.label); await testFundPk(page, viewport.label); await testIndexValuation(page, viewport.label);
