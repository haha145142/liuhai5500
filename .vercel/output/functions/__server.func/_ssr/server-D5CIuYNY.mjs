...
import { i as SECTOR_RULES, m as safeText, n as GLOBAL_DEFS, r as INDEX_DEFS } from "./format-9-Yv8U5-.mjs";
import { t as calcIndicators } from "./indicators-D24vzKbj.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/server-D5CIuYNY.js
...
async function fetchIndices() {
	...
	try {
		const arr = asArr((await emJson(`https://push2.eastmoney.com/api/qt/ulist.np/get?fltt=2&invt=2&fields=f12,f14,f2,f3,f4&secids=${secids}&ut=${EM_UT}&_=${Date.now()}`))?.data?.diff);
		if (arr.length) {
...
async function fetchBoards() {
	...
	async function clist(fs, type) {
		return asArr((await emJson(`https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=80&po=1&np=1&fltt=2&invt=2&fid=f3&fs=${encodeURIComponent(fs)}&fields=${fields}&ut=${EM_UT}&_=${Date.now()}`, 12e3))?.data?.diff).map((x) => ({
			code: String(x.f12 || ""),
			name: String(x.f14 || ""),
			type,
			change: n(x.f3),
...
async function fetchFlow() {
	try {
		const arr = asArr((await emJson(`https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=400&po=1&np=1&fltt=2&invt=2&fid=f62&fs=${encodeURIComponent("m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23")}&fields=f62,f66,f69,f72,f75&ut=${EM_UT}&_=${Date.now()}`, 12e3))?.data?.diff);
...
async function fetchGlobal() {
	try {
		const chunks = (await fetchText(`https://qt.gtimg.cn/q=${GLOBAL_DEFS.map((x) => x.tencent).join(",")}`, 8e3)).split(";");
...
var getFund = createServerFn({ method: "POST" }).validator((input) => input).handler(getFund_createServerFn_handler, async ({ data }) => {
	...
	const weekPct = periodPct(5);
	const monthPct = periodPct(20);
	return {
		...empty,
		code,
		name,
...
var getFundRank = createServerFn({ method: "POST" }).validator((input) => input).handler(getFundRank_createServerFn_handler, async ({ data }) => {
	...
	try {
		const rows = (parseMaybeJsonp(await fetchText(`https://fund.eastmoney.com/data/rankhandler.aspx?op=ph&dt=kf&ft=all&rs=&gs=0&sc=${sc}&st=desc&pi=1&pn=40&dx=1&_=${Date.now()}`, 12e3, { Referer: "https://fund.eastmoney.com/" }))?.datas || []).map((line) => {
...
//#endregion
export { analyzeMarket_createServerFn_handler, getFundRank_createServerFn_handler, getFund_createServerFn_handler, getNews_createServerFn_handler, getSnapshot_createServerFn_handler, searchFund_createServerFn_handler };
