//#region node_modules/.nitro/vite/services/ssr/assets/indicators-D24vzKbj.js
...
function calcIndicators(c) {
	...
	const ma = (p) => c.length < p ? null : c.slice(-p).reduce((a, b) => a + b, 0) / p;
	const ma5 = ma(5);
	const ma10 = ma(10);
	const ma20 = ma(20);
...
function calcSwingTrade(metrics, cost, price) {
	...
	const step = Math.max(1.5, Math.min(6, amplitude * .6));
	...
	const rsiFactor = rsi >= 30 && rsi <= 70 ? 1 : rsi >= 20 && rsi <= 80 ? .6 : .2;
...
//#endregion
export { calcSwingTrade as n, calcIndicators as t };
