//#region node_modules/.nitro/vite/services/ssr/assets/six-factor-_XDWLJ69.js
function calcSixFactor(s, benchPct) {
	const ch = s.change ?? 0;
	const fl = s.flow ?? 0;
	const trendVote = ch > .5 ? 1 : ch < -.5 ? -1 : 0;
	const momVote = ch > 1 ? 1 : ch < -1 ? -1 : 0;
	const flowYi = fl / 1e8;
	const fundVote = flowYi > 1 ? 1 : flowYi < -1 ? -1 : 0;
	const relDiff = ch - (benchPct ?? 0);
	const relVote = relDiff > .3 ? 1 : relDiff < -.3 ? -1 : 0;
	let volVote = 0;
	if (Math.abs(ch) > 4) volVote = -1;
	else if (Math.abs(ch) > 2.5) volVote = -.5;
	const st = s.streak || 0;
	let streakVote = 0;
	if (st >= 3) streakVote = -.5;
	else if (st <= -3) streakVote = .5;
	const votes = [
		trendVote,
		momVote,
		fundVote,
	...
	const trendLabel = ch > 1 ? "强" : ch > 0 ? "偏强" : ch < -1 ? "弱" : ch < -.5 ? "偏弱" : "震荡";
