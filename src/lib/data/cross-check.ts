    ...
    try {
      ...
      const validated = data.indices.map((primary) => {
        ...
        const priceGap = primary.price != null && backup.price != null && primary.price !== 0
          ...
          : 0;
        // Provider timing/rounding can cause small differences. On a larger
        // disagreement, keep the primary value instead of blanking the card;
        // the caller can surface the disagreement in the validation note.
      ...
      return {
        ...
        note: compared >= 3
          ? disputed > 0
            ? `东方财富 + 腾讯财经交叉核验 ${compared} 项；${disputed} 项有分歧，保留主源值`
            : `东方财富 + 腾讯财经交叉核验 ${compared} 项`
          : "第二行情源无法完成足够交叉核验",
      };
    } catch {
