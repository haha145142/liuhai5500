...
export const getFundSectorQuotes=createServerFn({method:"POST"}).validator((input:{ids?:string[]})=>input).handler(async({data}):Promise<{rows:FundSectorQuote[];fetchedAt:number;weekend:boolean}>=>{
 const ids=(data.ids?.length?data.ids:DEFAULT_FUND_SECTOR_IDS).filter(id=>FUND_SECTORS.some(s=>s.id===id));const key=ids.join(",");const closed=isExchangeClosed();const ttl=closed?24*60*60*1000:CACHE_TTL;
 ...
 if(lastGood&&lastGood.key===key){
   const staleRows=lastGood.data.map(r=>({...r,validation:r.validCount>0?"cached_latest_trading_day" as const:"unavailable" as const,source:r.validCount>0?`上一可靠数据 · ${lastGood?.data.find(x=>x.id===r.id)?.marketDate??"最近交易日"}`:"当前暂无新数据"}));
   return{rows:staleRows,fetchedAt:lastGood.ts,weekend:true};
 }
