...
function mergeQuote(primary:ReturnType<typeof pick>|null,secondary:ReturnType<typeof pick>|null,own:Awaited<ReturnType<typeof getCalculatedFund>>|null,code:string,fallbackName:string):FundSectorFundQuote{
 ...
 if(ownPct!=null||own?.nav!=null){const isEstimate=own?.estimatePct!=null;return{code,name:own?.name||fallbackName,pct:ownPct,nav:own?.nav??primary?.nav??secondary?.nav??null,estimate:own?.estimate??primary?.estimate??secondary?.estimate??null,time:own?.estimateTime??primary?.time??secondary?.time??null,date:own?.navDate??primary?.date??secondary?.date??null,validation:isEstimate&&((own?.quoteCrossCheckedWeight??0)>0)?"cross_checked":"single_source",source:isEstimate?"Fund AI Pro 自算盘中估值":"最近官方净值"};}
 if(primary&&secondary){const ok=primary.pct!=null&&secondary.pct!=null&&Math.abs(primary.pct-secondary.pct)<=0.15;return{code,name:primary.name||secondary.name||fallbackName,pct:primary.pct??secondary.pct,nav:primary.nav??secondary.nav,estimate:primary.estimate??secondary.estimate,time:primary.time??secondary.time,date:primary.date??secondary.date,validation:ok?"cross_checked":"single_source",source:ok?"天天基金 + 东方财富交叉验证":"双源有分歧，已降级"};}
