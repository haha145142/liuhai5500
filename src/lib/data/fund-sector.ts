...
type BoardFlow = {
  code:string; name:string; change:number|null; flow:number|null;
  super:number|null; large:number|null; mid:number|null; small:number|null; turnover:number|null;
};
...
async function fetchBoardFlows():Promise<Map<string,BoardFlow>>{
  try{
    const j=parseMaybeJsonp(await fetchText(`https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=1200&po=1&np=1&fltt=2&invt=2&fid=f3&fs=${encodeURIComponent("m:90+t:2,m:90+t:3")}&fields=${BOARD_FIELDS}&ut=${EM_UT}&_=${Date.now()}`,10000,{Referer:"https://data.eastmoney.com/bkzj/"})) as any;
    const arr=Array.isArray(j?.data?.diff)?j.data.diff:[];
    return new Map(arr.map((x:any)=>[String(x.f12||""),{
      code:String(x.f12||""),name:String(x.f14||""),change:pct(x.f3),flow:money(x.f62),
      super:money(x.f66),large:money(x.f69),mid:money(x.f72),small:money(x.f75),turnover:money(x.f6)
    } as BoardFlow]).filter(([code])=>!!code));
  }catch{return new Map();}
}

function flowSignal(change:number|null,flow:number|null):FundSectorQuote["flowSignal"]{
  if(change==null||flow==null)return "unavailable";
  if(Math.abs(flow)<1)return "neutral";
  if(change<0&&flow<0)return "outflow";
  if(change>0&&flow>0)return "inflow";
  return "mixed";
}
...
function mergeQuote(primary:ReturnType<typeof pick>|null,secondary:ReturnType<typeof pick>|null,own:Awaited<ReturnType<typeof getCalculatedFund>>|null,code:string,fallbackName:string):FundSectorFundQuote{
 const providerPct=primary?.pct!=null&&secondary?.pct!=null?Math.abs(primary.pct-secondary.pct)<=0.15?((primary.pct+secondary.pct)/2):null:null;
 ...
 return {code,name:fallbackName,pct:null,nav:null,estimate:null,time:null,date:null,validation:"unavailable",source:"当前暂无新数据"};
}
...
export const getFundSectorQuotes=createServerFn({method:"POST"}).validator((input:{ids?:string[]})=>input).handler(async({data}):Promise<{rows:FundSectorQuote[];fetchedAt:number;weekend:boolean}>=>{
 const ids=(data.ids?.length?data.ids:DEFAULT_FUND_SECTOR_IDS).filter(id=>FUND_SECTORS.some(s=>s.id===id));const key=ids.join(",");const closed=isExchangeClosed();const ttl=closed?CLOSED_CACHE_TTL:CACHE_TTL;
 if(cache&&cache.key===key&&Date.now()-cache.ts<ttl)return{rows:cache.data,fetchedAt:cache.ts,weekend:closed};
 ...
 const [ownMap,providerPairs,boardFlows]=await Promise.all([
   getOwnQuotes(codes),
   Promise.all(chunk(codes,40).map(async batch=>({east:await fetchProvider(EM,batch),tt:await fetchProvider(TT,batch)}))),
   fetchBoardFlows()
 ]);
 ...
 const rows=sectors.map((sector):FundSectorQuote=>{
   const funds=sector.funds.map(f=>mergeQuote(eastMap.get(f.code)??null,ttMap.get(f.code)??null,ownMap.get(f.code)??null,f.code,f.name));
   ...
   const validation:FundSectorQuote["validation"]=!valid.length?"unavailable":cross>=Math.max(1,Math.ceil(valid.length*0.6))?"cross_checked":"single_source";
   const dates=valid.map(f=>f.date).filter((v):v is string=>!!v);
   const latest=dates.sort().at(-1)??tradingDateLabel();
   const coveragePct=sector.funds.length?valid.length/sector.funds.length*100:0;
   const crossCheckedPct=valid.length?cross/valid.length*100:0;
   const rule=FUND_SECTORS.find(s=>s.id===sector.id);
   const matchingName=sector.name.replace(/芯片|行业|产业/g,"");
   const flow=([...(boardFlows.values())].find(b=>rule&&b.code===({semi:"BK0917"} as Record<string,string>)[sector.id])
      ?? [...boardFlows.values()].find(b=>b.name===sector.name)
      ?? [...boardFlows.values()].find(b=>matchingName.length>1&&b.name.includes(matchingName))) ?? null;
   const marketPct=flow?.change??null;
   const mainFlow=flow?.flow??null;
   const signal=flowSignal(marketPct,mainFlow);
   return {
     id:sector.id,name:sector.name,icon:sector.icon,pct:avg,up,down,flat,validCount:valid.length,totalCount:sector.funds.length,
     leader:sorted[0]??null,weakest:sorted.at(-1)??null,funds,marketDate:latest,validation,
     source:validation==="cross_checked"?"自算估值 + 多源交叉验证":validation==="single_source"?"当前/最近有效数据":"当前暂无新数据",
     coveragePct,crossCheckedPct,crossCheckedCount:cross,
     marketPct,mainFlow,superFlow:flow?.super??null,largeFlow:flow?.large??null,midFlow:flow?.mid??null,smallFlow:flow?.small??null,turnover:flow?.turnover??null,
     flowSignal:signal,flowValidation:flow?"live":"unavailable"
   };
 });
 if(rows.some(r=>r.validCount>0)){cache={key,ts:Date.now(),data:rows};lastGood={key,ts:Date.now(),data:rows};return{rows,fetchedAt:Date.now(),weekend:closed};}
 if(lastGood&&lastGood.key===key&&Date.now()-lastGood.ts<STALE_CACHE_TTL){return{rows:lastGood.data.map(r=>({...r,validation:r.validCount>0?"cached_latest_trading_day":"unavailable",flowValidation:r.mainFlow!=null?"cached":"unavailable",source:r.validCount>0?`上一可靠数据 · ${r.marketDate??tradingDateLabel(lastGood?.ts?new Date(lastGood.ts):new Date())}`:"当前暂无新数据"})),fetchedAt:lastGood.ts,weekend:closed};}
 return{rows,fetchedAt:Date.now(),weekend:closed};
});
