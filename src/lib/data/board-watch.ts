...
export type BoardWatchQuote = { code: string; name: string; icon: string; pct: number | null; mainFlow: number | null; superFlow: number | null; largeFlow: number | null; midFlow: number | null; smallFlow: number | null; turnover: number | null; marketDate: string; source: string; validation: "live" | "recent" | "unavailable"; priceStatus: "live" | "recent" | "unavailable"; flowStatus: "fresh" | "stale" | "unavailable"; flowScore: number | null; flowSignal: "强流入" | "流入" | "中性" | "流出" | "强流出" | "暂无" };

const UT = "fa5fd1943c7b386f172d6893dbfba10b";
const SEARCH_FIELDS = "f12,f14";
const QUOTE_FIELDS = "f12,f14,f3,f6,f62,f66,f72,f78,f84";
const LIMIT = 1200;
...
function resolveBoardName(code: string, remoteName?: string | null) {
  const remote = String(remoteName ?? "").trim();
  if (remote && !/^BK\d+$/i.test(remote)) return remote;
  const known = KNOWN.get(code);
  return known?.name || SECTOR_RULES.find((s) => s.bkCode === code)?.name || "";
}
...
export const searchFundBoards = createServerFn({ method: "POST" }).validator((input: { query?: string }) => input).handler(async ({ data }): Promise<{ items: BoardCandidate[] }> => {
  ...
  const remote = rows.map((row): (BoardCandidate & { _score: number }) | null => {
    ...
    const name = resolveBoardName(code, rowName(row));
...
export const getBoardWatchQuotes = createServerFn({ method: "POST" }).validator((input: { codes?: string[] }) => input).handler(async ({ data }): Promise<{ rows: BoardWatchQuote[]; fetchedAt: number; weekend: boolean }> => {
  ...
  // An array of empty rows is not a successful response; let the browser fallback run.
  const usableRows = result.filter((row) => row.pct != null || row.mainFlow != null || row.superFlow != null || row.largeFlow != null || row.midFlow != null || row.smallFlow != null || row.turnover != null);
  return { rows: usableRows, fetchedAt: Date.now(), weekend: closed };
});
