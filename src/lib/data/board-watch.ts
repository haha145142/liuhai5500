...
function resolveBoardName(code: string, remoteName?: string | null) { const remote = String(remoteName ?? "").trim(); if (remote && !/^BK\d+$/i.test(remote)) return remote; const known = KNOWN.get(code); return known?.name || SECTOR_RULES.find((s) => s.bkCode === code)?.name || ""; }
