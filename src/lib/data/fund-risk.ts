...
function calcMaxDrawdown(rows: Record<string, unknown>[]) {
  const navs = rows
    .map((row) => ({ date: String(row.FSRQ ?? "").trim(), nav: n(row.DWJZ) }))
    .filter((x): x is { date: string; nav: number } => !!x.date && x.nav != null && x.nav > 0)
...
async function loadOne(code: string): Promise<FundRiskMetric> {
  ...
  try {
    const url = `https://api.fund.eastmoney.com/f10/lsjz?fundCode=${encodeURIComponent(code)}&pageIndex=1&pageSize=260&startDate=&endDate=&_=${Date.now()}`;
    const raw = await fetchText(url, 5000, { Referer: `https://fundf10.eastmoney.com/jjjz_${code}.html` });
    const rows = extractRows(parseMaybeJsonp(raw));
    const metric = calcMaxDrawdown(rows);
    const value: FundRiskMetric = {
      code,
      maxDrawdown1Y: metric.drawdown,
      historyDate: metric.date,
      source: rows.length ? "东方财富历史净值" : "暂无可靠历史净值",
    };
    ...
    return {
      code,
      maxDrawdown1Y: cachedValue?.value.maxDrawdown1Y ?? null,
      historyDate: cachedValue?.value.historyDate ?? null,
      source: cachedValue?.value.source ?? "暂无可靠历史净值",
    };
  }
}
...
export const getFundRiskMetrics = createServerFn({ method: "POST" })
  .validator((input: { codes: string[] }) => ({
    codes: Array.isArray(input?.codes)
      ? input.codes.map((code) => String(code).trim()).filter((code) => /^\d{6}$/.test(code)).slice(0, 40)
      : [],
  }))
  .handler(async ({ data }): Promise<FundRiskMetric[]> => {
    ...
    return data.codes.map((code) => results.find((item) => item.code === code) ?? {
      code,
      maxDrawdown1Y: null,
      historyDate: null,
      source: "暂无可靠历史净值",
