...
async function fetchBoards(): Promise<{ sectors: SectorQuote[]; boards: BoardQuote[]; source: DataSource }> {
  ...
  try {
    ...
    const sectors = SECTOR_RULES.map((r) => { const hit = all.find((x) => x.code === r.bkCode) || all.find((x) => x.name === r.name) || all.find((x) => r.searchKeys.some((k) => x.name.includes(k))); return { id: r.id, name: r.name, bkCode: r.bkCode, change: hit?.change ?? null, flow: hit?.flow ?? null, super: hit?.super ?? null, large: hit?.large ?? null, mid: hit?.mid ?? null, small: hit?.small ?? null, turnover: hit?.turnover ?? null, available: hit?.change != null, streak: 0, etfCode: r.etf?.code, etfName: r.etf?.name }; });
...
export const getSnapshot = createServerFn({ method: "GET" }).handler(async (): Promise<Snapshot> => {
  const hit = cached<Snapshot>("snap", 20_000, null); if (hit) return hit;
  const [idx, boards, flow, global] = await Promise.all([fetchIndices(), fetchBoards(), fetchFlow(), fetchGlobal()]);
  return cached("snap", 20_000, { indices: idx.list, sectors: boards.sectors, boards: boards.boards.slice(0, 40), flow: flow.flow, global: global.list, sources: [idx.source, boards.source, flow.source, global.source], fetchedAt: Date.now() })!;
});
...
export const getNews = createServerFn({ method: "GET" }).handler(async (): Promise<NewsFeed> => {
  const hit = cached<NewsFeed>("news", 60_000, null); if (hit) return hit;
...
export const getFund = createServerFn({ method: "POST" }).validator((input: { code: string }) => input).handler(async ({ data }): Promise<FundQuote> => {
  ...
  try {
    ...
    const rawRows = j?.Data?.LSJZList || [];
    for (const r of rawRows.slice().reverse()) { const v = n(r.DWJZ); if (v == null) continue; history.push(v); historyPoints.push({ date: String(r.FSRQ || ""), nav: v, changePct: n(r.JZZZL) }); }
    const last = rawRows[0]; if (last) { nav = n(last.DWJZ); navDate = String(last.FSRQ || "") || null; dayPct = n(last.JZZZL); source = "东方财富历史净值"; }
  } catch {}
  try {
...
export const analyzeMarket = createServerFn({ method: "POST" }).validator((input: { prompt: string }) => input).handler(async ({ data }) => {
  const apiKey = process.env.XAI_API_KEY; if (!apiKey) return { ok: false as const, error: "AI 暂不可用", text: "" };
  ...
  const body = (await res.json()) as { choices?: { message?: { content?: string } }[] }; return { ok: true as const, error: "", text: body.choices?.[0]?.message?.content ?? "" };
});

export const analyzeNews = createServerFn({ method: "POST" }).validator((input: { prompt: string }) => input).handler(async ({ data }) => {
  ...
  try {
    const res = await fetch("https://api.x.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "grok-4.5", max_tokens: 1000, temperature: 0.15, messages: [
        { role: "system", content: "你是基金投资者的新闻解读助手。只使用输入中的新闻、发布时间、板块、指数和资金证据。绝不补造事实、数字、时间或来源；没有证据就明确写‘暂无可靠数据’。必须区分新闻事实与市场推测。不要机械复述标题，不要喊单，不给确定性买卖建议。用简洁白话中文。输出结构固定为：【今日新闻结论】【最重要的新闻】【板块影响】【与我的持仓关系】【一句话提醒】。每条重要新闻写：发生了什么｜为什么重要｜影响谁｜利好/利空/中性｜证据是否验证。无法判断就写‘暂无可靠数据’。" },
