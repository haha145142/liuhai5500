...
function chinaDateLabel(date = new Date()) { const shifted = new Date(date.getTime() + 8 * 60 * 60 * 1000); return `${shifted.getUTCFullYear()}-${String(shifted.getUTCMonth() + 1).padStart(2, "0")}-${String(shifted.getUTCDate()).padStart(2, "0")}`; }
...
function isSameDayOfficial(quote: FundQuote | null | undefined) { return !!quote && quote.officialNavPublished === true && quote.nav != null && quote.navDate === chinaDateLabel(); }
function canUseCachedFund(quote: FundQuote | null | undefined, phase: ReturnType<typeof getMarketPhase>) {
  if (!quote || !usableFundData(quote)) return false;
  if (phase === "morning" || phase === "afternoon" || phase === "lunch") return isSameDayEstimate(quote) || isSameDayOfficial(quote);
  return true;
}

async function rememberFundCode(code: string) {
  if (!/^\d{6}$/.test(code)) return;
  try { const cached = await sharedCacheGet<string[]>(FUND_REGISTRY_KEY); const existing = Array.isArray(cached?.value) ? cached.value.filter((x) => /^\d{6}$/.test(String(x))) : []; const next = [code, ...existing.filter((x) => x !== code)].slice(0, 200); await sharedCacheSet(FUND_REGISTRY_KEY, next, FUND_REGISTRY_TTL_MS); } catch {}
}
export async function getPrewarmFundCodes(): Promise<string[]> { const cached = await sharedCacheGet<string[]>(FUND_REGISTRY_KEY); return Array.isArray(cached?.value) ? cached.value.filter((x) => /^\d{6}$/.test(String(x))).slice(0, 200) : []; }
...
async function loadFundQuote(code: string): Promise<FundQuote> {
  ...
  if (phase === "postclose") { const saved = postCloseEstimate.get(code); if (saved && isSameDayEstimate(saved)) return saved; }
...
export async function refreshFundQuote(code: string): Promise<FundQuote> {
  const normalizedCode = code.trim(); await rememberFundCode(normalizedCode); const value = normalizeFundValuationState(await loadFundQuote(normalizedCode));
  if (usableFundData(value)) { fundResolved.set(normalizedCode, { at: Date.now(), value }); if (isSameDayEstimate(value)) postCloseEstimate.set(normalizedCode, value); await sharedCacheSet(`fund-ai-pro:fund:${normalizedCode}`, value, isSameDayEstimate(value) ? FUND_ESTIMATE_SHARED_CACHE_TTL_MS : FUND_SHARED_CACHE_TTL_MS); }
  return value;
}

export const getFund = async ({ data }: { data: { code: string } }): Promise<FundQuote> => {
  const code = data.code.trim(); void rememberFundCode(code); const phase = getMarketPhase(); const localTtl = phase === "postclose" ? FUND_POSTCLOSE_TTL_MS : FUND_CACHE_TTL_MS;
  const cached = fundResolved.get(code);
  if (cached && Date.now() - cached.at < localTtl && canUseCachedFund(cached.value, phase)) return normalizeFundValuationState(cached.value);
  const running = fundPending.get(code); if (running) return running;
  const shared = await sharedCacheGet<FundQuote>(`fund-ai-pro:fund:${code}`);
  if (shared?.value && canUseCachedFund(shared.value, phase)) {
    const normalized = normalizeFundValuationState(shared.value); const sameDayEstimate = isSameDayEstimate(normalized);
    fundResolved.set(code, { at: Date.now(), value: normalized }); if (sameDayEstimate) postCloseEstimate.set(code, normalized); return normalized;
  }
  const request = loadFundQuote(code).then((value) => {
    const normalized = normalizeFundValuationState(value);
    if (usableFundData(normalized)) { fundResolved.set(code, { at: Date.now(), value: normalized }); if (isSameDayEstimate(normalized)) postCloseEstimate.set(code, normalized); void sharedCacheSet(`fund-ai-pro:fund:${code}`, normalized, isSameDayEstimate(normalized) ? FUND_ESTIMATE_SHARED_CACHE_TTL_MS : FUND_SHARED_CACHE_TTL_MS).catch(() => {}); }
    return normalized;
...
async function readFieldCachedSnapshot(): Promise<Snapshot | null> {
  const [indices, sectors, flow, global, meta] = await Promise.all([
    sharedCacheGet<SnapshotField<Snapshot["indices"]>>("fund-ai-pro:snapshot:indices"), sharedCacheGet<SnapshotField<Snapshot["sectors"]>>("fund-ai-pro:snapshot:sectors"), sharedCacheGet<SnapshotField<Snapshot["flow"]>>("fund-ai-pro:snapshot:flow"), sharedCacheGet<SnapshotField<Snapshot["global"]>>("fund-ai-pro:snapshot:global"), sharedCacheGet<SnapshotMeta>("fund-ai-pro:snapshot:meta"),
  ]);
  if (!meta?.value) return null;
  const generation = meta.value.generation; const same = (entry: { value?: SnapshotField<unknown> } | null | undefined) => entry?.value?.generation === generation;
  const coherentIndices = same(indices) ? indices!.value!.value : null; const coherentSectors = same(sectors) ? sectors!.value!.value : null; const coherentFlow = same(flow) ? flow!.value!.value : null; const coherentGlobal = same(global) ? global!.value!.value : null;
...
export const getSnapshot = async () => { if (snapshotPending) return snapshotPending; snapshotPending = (async () => { const cached = await readFieldCachedSnapshot(); if (cached && cached.indices.length && cached.sectors.length && cached.sources.length) return cached; const snapshot = await buildSnapshot(); cacheSnapshotFields(snapshot); return snapshot; })().finally(() => { snapshotPending = null; }); return snapshotPending; };
