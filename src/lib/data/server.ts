...
async function fetchBoards(): Promise<{ sectors: SectorQuote[]; boards: BoardQuote[]; source: DataSource }> {
  ...
  try {
    ...
    const sectors = SECTOR_RULES.map((r) => { const hit = all.find((x) => x.code === r.bkCode) || all.find((x) => x.name === r.name) || all.find((x) => r.searchKeys.some((k) => x.name.includes(k))); return { id: r.id, name: r.name, bkCode: r.bkCode, change: hit?.change ?? null, flow: hit?.flow ?? null, super: hit?.super ?? null, large: hit?.large ?? null, mid: hit?.mid ?? null, small: hit?.small ?? null, turnover: hit?.turnover ?? null, available: hit?.change != null, streak: 0, etfCode: r.etf?.code, etfName: r.etf?.name, validation: hit?.change != null ? "single_source" as const : "unavailable" as const }; });
...
async function fetchFlow(): Promise<{ flow: MarketOrder | null; source: DataSource }> {
  try {
    ...
    const main = sum("f62"), super = sum("f66"), large = sum("f69"), mid = sum("f72"), small = sum("f75");
    const gap = Math.abs(main - (super + large));
    const consistent = gap <= Math.max(50_000_000, Math.abs(main) * 0.12);
    return { flow: { main, super, large, mid, small, count: arr.length }, source: src("资金", true, `东方财富全A抽样 ${arr.length} 只 · ${consistent ? "主力口径一致" : "主力与大单拆分存在偏差"}`) };
...
export const getSnapshot = createServerFn({ method: "GET" }).handler(async (): Promise<Snapshot> => {
  ...
  const sources = [idx.source, boards.source, flow.source, global.source];
  const anyMarket = idx.list.some((x) => x.pct != null) || boards.sectors.some((x) => x.change != null) || !!flow.flow;
  const validation: Snapshot["validation"] = weekend
    ? "cached_latest_trading_day"
    : idx.validation === "cross_checked" && anyMarket
      ? "cross_checked"
      : anyMarket ? "single_source" : "cached_latest_trading_day";
  const marketDate = weekend ? tradingDateLabel() : new Date().toISOString().slice(0, 10);
  return cached("snap", ttl, { indices: idx.list, sectors: boards.sectors, boards: boards.boards.slice(0, 40), flow: flow.flow, global: global.list, sources, fetchedAt: Date.now(), marketDate, validation })!;
});
...
export const getNews = createServerFn({ method: "GET" }).handler(async (): Promise<NewsFeed> => { const hit = cached<NewsFeed>("news", 60_000, null); if (hit) return hit; const fetchedAt = Date.now(); const [ths, em, wscn, guba] = await Promise.all([fetchTHS(fetchedAt), fetchEmFlash(fetchedAt), fetchWscn(fetchedAt), fetchGuba(fetchedAt)]); const seen = new Set<string>(); const items: NewsItem[] = []; for (const n0 of [...ths, ...em, ...wscn]) { if (seen.has(n0.title)) continue; seen.add(n0.title); items.push(n0); } items.sort((a, b) => (b.publishedAt || 0) - (a.publishedAt || 0)); const latest = items.map((x) => x.publishedAt).filter((x): x is number => x != null && x > 0); return cached("news", 60_000, { items: items.slice(0, 50), deep: items.filter((x) => x.summary.length > 40).slice(0, 12), sentiment: guba.slice(0, 12), sources: [src("同花顺", ths.length > 0, ths.length ? `${ths.length} 条` : "暂不可用"), src("东财快讯", em.length > 0, em.length ? `${em.length} 条` : "暂不可用"), src("华尔街见闻", wscn.length > 0, wscn.length ? `${wscn.length} 条` : "暂不可用"), src("社区情绪", guba.length > 0, guba.length ? `${guba.length} 条` : "暂不可用")], fetchedAt, latestPublishedAt: latest.length ? Math.max(...latest) : null })!; });
export const getFund = createServerFn({ method: "POST" }).validator((input: { code: string }) => input).handler(async ({ data }): Promise<FundQuote> => { const code = data.code.replace(/\D/g, "").slice(0, 6); const empty: FundQuote = { code, name: code, type: "基金", nav: null, navDate: null, estimate: null, estimatePct: null, estimateTime: null, dayPct: null, weekPct: null, monthPct: null, history: [], historyPoints: [], metrics: null, source: "数据源暂不可用" }; if (!/^\d{6}$/.test(code)) return empty; let name = code, nav: number | null = null, navDate: string | null = null, dayPct: number | null = null, estimate: number | null = null, estimatePct: number | null = null, estimateTime: string | null = null, source = "数据源暂不可用"; const history: number[] = []; const historyPoints: FundHistoryPoint[] = []; try { const j = (await emJson(`https://api.fund.eastmoney.com/f10/lsjz?fundCode=${code}&pageIndex=1&pageSize=400`, 12000)) as { Data?: { LSJZList?: Record<string, unknown>[] } }; const rawRows = j?.Data?.LSJZList || []; for (const r of rawRows.slice().reverse()) { const v = n(r.DWJZ); if (v == null) continue; history.push(v); historyPoints.push({ date: String(r.FSRQ || ""), nav: v, changePct: n(r.JZZZL) }); } const last = rawRows[0]; if (last) { nav = n(last.DWJZ); navDate = String(last.FSRQ || "") || null; dayPct = n(last.JZZZL); source = "东方财富历史净值"; } } catch {} try { const gz = parseMaybeJsonp(await fetchText(`https://fundgz.1234567.com.cn/js/${code}.js?rt=${Date.now()}`, 8000, { Referer: "https://fund.eastmoney.com/" })) as Record<string, unknown> | null; if (gz) { name = String(gz.name || name); if (nav == null) nav = n(gz.dwjz); if (!navDate) navDate = String(gz.jzrq || "") || null; estimate = n(gz.gsz); estimatePct = n(gz.gszzl); estimateTime = String(gz.gztime || "") || null; source = source === "数据源暂不可用" ? "天天基金估值" : source + " + 天天基金估值"; } } catch {} if (name === code) { try { const j = (await emJson(`https://fundsuggest.eastmoney.com/FundSearch/api/FundSearchAPI.ashx?m=1&key=${code}`, 6000)) as { Datas?: { CODE?: string; NAME?: string; CATEGORYDESC?: string }[] }; const hit = (j?.Datas || []).find((x) => x.CODE === code) || j?.Datas?.[0]; if (hit?.NAME) name = hit.NAME; if (hit?.CATEGORYDESC) empty.type = hit.CATEGORYDESC; } catch {} } const metrics = calcIndicators(history); const latest = history[history.length - 1] ?? null; const periodPct = (days: number) => { if (latest == null || history.length <= days) return null; const base = history[history.length - 1 - days]; return base ? ((latest - base) / base) * 100 : null; }; const now = new Date(Date.now() + 8 * 60 * 60 * 1000); const today = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}-${String(now.getUTCDate()).padStart(2, "0")}`; const officialNavPublished = !!navDate && navDate === today && nav != null; const valuationStatus: FundQuote["valuationStatus"] = officialNavPublished ? "official_nav" : estimate != null ? "estimate" : nav != null ? "stale" : "unavailable"; return { ...empty, code, name, nav, navDate, estimate, estimatePct, estimateTime, dayPct, weekPct: periodPct(5), monthPct: periodPct(20), history, historyPoints, metrics, source, officialNavPublished, valuationStatus, estimateConfidence: estimate != null ? "medium" : undefined }; });
export const searchFund = createServerFn({ method: "POST" }).validator((input: { q: string }) => input).handler(async ({ data }): Promise<{ code: string; name: string; type: string }[]> => { const q = data.q.trim(); if (!q) return []; try { const j = (await emJson(`https://fundsuggest.eastmoney.com/FundSearch/api/FundSearchAPI.ashx?m=1&key=${encodeURIComponent(q)}`, 8000)) as { Datas?: { CODE?: string; NAME?: string; CATEGORYDESC?: string }[] }; return (j?.Datas || []).filter((x) => x.CODE && x.NAME).slice(0, 8).map((x) => ({ code: String(x.CODE), name: String(x.NAME), type: String(x.CATEGORYDESC || "基金") })); } catch { return []; } });
export const getFundRank = createServerFn({ method: "POST" }).validator((input: { sort?: string }) => input).handler(async ({ data }): Promise<{ rows: RankRow[]; source: string; fetchedAt: number }> => { const sort = data.sort || "r"; const sc = sort === "z" ? "zzf" : sort === "1n" ? "1nzf" : sort === "6y" ? "6yzf" : "rzf"; try { const text = await fetchText(`https://fund.eastmoney.com/data/rankhandler.aspx?op=ph&dt=kf&ft=all&rs=&gs=0&sc=${sc}&st=desc&pi=1&pn=40&dx=1&_=${Date.now()}`, 12000, { Referer: "https://fund.eastmoney.com/" }); const j = parseMaybeJsonp(text) as { datas?: string[] } | null; const rows: RankRow[] = (j?.datas || []).map((line) => { const a = String(line).split(","); return { code: a[0] || "", name: a[1] || "", nav: n(a[4]), day: n(a[6]), week: n(a[7]), month: n(a[8]), ytd: n(a[14]) }; }).filter((x) => x.code && x.name); return { rows, source: rows.length ? "天天基金/东方财富排行" : "数据源暂不可用", fetchedAt: Date.now() }; } catch { return { rows: [], source: "数据源暂不可用", fetchedAt: Date.now() }; } });
export const analyzeMarket = createServerFn({ method: "POST" }).validator((input: { prompt: string }) => input).handler(async ({ data }) => { const apiKey = process.env.XAI_API_KEY; if (!apiKey) return { ok: false as const, error: "AI 暂不可用", text: "" }; const res = await fetch("https://api.x.ai/v1/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model: "grok-4.5", max_tokens: 700, messages: [{ role: "system", content: "你是严谨的基金投研助手。只用用户提供的证据，没有就写「暂无可靠数据」，绝不编造数字。用大白话中文。不构成投资建议。按7步：发生了什么/市场反应/资金确认/新闻催化/政策支持/外围共振/最后判断。" }, { role: "user", content: data.prompt.slice(0, 6000) }] }) }); if (!res.ok) return { ok: false as const, error: `AI 接口 ${res.status}`, text: "" }; const body = (await res.json()) as { choices?: { message?: { content?: string } }[] }; return { ok: true as const, error: "", text: body.choices?.[0]?.message?.content ?? "" }; });
export const analyzeNews = createServerFn({ method: "POST" }).validator((input: { prompt: string }) => input).handler(async ({ data }) => { const apiKey = process.env.XAI_API_KEY; if (!apiKey) return { ok: false as const, error: "AI 暂不可用：未配置 XAI_API_KEY", text: "" }; const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 25_000); try { const res = await fetch("https://api.x.ai/v1/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` }, signal: controller.signal, body: JSON.stringify({ model: "grok-4.5", max_tokens: 1000, temperature: 0.15, messages: [{ role: "system", content: "你是基金投资者的新闻解读助手。只使用输入中的新闻、发布时间、板块、指数和资金证据。绝不补造事实、数字、时间或来源；没有证据就明确写‘暂无可靠数据’。必须区分新闻事实与市场推测。不要机械复述标题，不要喊单，不给确定性买卖建议。用简洁白话中文。输出结构固定为：【今日新闻结论】【最重要的新闻】【板块影响】【与我的持仓关系】【一句话提醒】。每条重要新闻写：发生了什么｜为什么重要｜影响谁｜利好/利空/中性｜证据是否验证。无法判断就写‘暂无可靠数据’。" }, { role: "user", content: data.prompt.slice(0, 10000) }] }) }); if (!res.ok) return { ok: false as const, error: `AI 接口 ${res.status}`, text: "" }; const body = (await res.json()) as { choices?: { message?: { content?: string } }[] }; const text = body.choices?.[0]?.message?.content?.trim() ?? ""; return text ? { ok: true as const, error: "", text } : { ok: false as const, error: "AI 未返回有效解读", text: "" }; } catch (e) { return { ok: false as const, error: e instanceof Error && e.name === "AbortError" ? "AI 解读超时，请稍后重试" : "AI 解读暂时不可用", text: "" }; } finally { clearTimeout(timer); } });
