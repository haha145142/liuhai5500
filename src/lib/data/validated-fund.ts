...
export const getValidatedFund=createServerFn({method:"POST"}).validator((input:{code:string})=>input).handler(async({data}):Promise<FundQuote>=>{
  ...
  if(!usable.length)return{...base,estimate:null,estimatePct:null,estimateConfidence:"low",valuationStatus:base.nav!=null?"waiting_official_nav":"unavailable",source:"重仓实时行情暂无可靠数据",estimateValidation:"三源实时行情均未达到可靠门槛"};
  ...
  return{...base,estimate,estimatePct:pct,estimateConfidence:confidenceFrom(stats),estimateCoverage:coverage,usableWeight:stats.usableWeight,source:`自有穿透估值 · 三源实时行情校验 · 健康${health}`,estimateValidation:`三源一致 ${stats.three} · 双源一致 ${stats.two} · 单源 ${stats.single} · 分歧 ${stats.disputed} · 覆盖 ${coverage.toFixed(1)}% · 一致度 ${agreement}`,liveHoldings:holdings.map(h=>{const hit=validated.find(x=>x.holding.code===h.code);if(!hit)return h;const q=hit.quote;const label=q.agreement==="three_source"?"三源一致":q.agreement==="two_source"?"双源一致":q.agreement==="single_source"?"单源可用":q.agreement==="disputed"?"来源分歧":"暂无可靠行情";return{...h,price:q.price,pct:q.pct,source:label};})}as FundQuote;
});
