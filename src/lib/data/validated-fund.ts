...
export const getValidatedFund=createServerFn({method:"POST"}).validator((input:{code:string})=>input).handler(async({data}):Promise<FundQuote>=>{
  ...
  if(!usable.length)return{...base,estimate:null,estimatePct:null,estimateConfidence:"low",valuationStatus:base.nav!=null?"waiting_official_nav":"unavailable",source:"重仓实时行情暂无可靠交叉验证",estimateValidation:"三源/双源实时行情均未达到可靠门槛"};
