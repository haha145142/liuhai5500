...
export function isOfficialNavToday(fund: FundQuote | undefined, now = new Date()) {
  ...
  return fund.officialNavPublished === true && sameChinaDate(fund.navDate, now);
}
...
export function selectFundDisplayQuote(fund: FundQuote|undefined,now=new Date()):FundDisplayQuote{
  ...
  if(isOfficialNavToday(fund,now))return{mode:"official_today",price:fund.nav,pct:resolveNavPct(fund,fund.navDate),label:"最近交易日官方净值",dataDate:fund.navDate,confidence:"high",reason:"数据层已确认当日官方净值已发布；非交易日继续沿用最近交易日官方净值"};
