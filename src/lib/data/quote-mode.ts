...
import { tradingDateLabel } from "./trading-day.ts";
...
export function isOfficialNavToday(fund: FundQuote | undefined, now = new Date()) {
  if (fund?.nav == null || fund.nav <= 0) return false;
  if (fund.officialNavPublished === true && sameChinaDate(fund.navDate, now)) return true;
  const phase = getMarketPhase(now);
  return phase === "postclose" && fund.navDate === tradingDateLabel(now);
}
...
function latestOfficial(fund: FundQuote): FundDisplayQuote { const price=fund.referenceNav??fund.nav; const date=fund.referenceNavDate??fund.navDate; return {mode:"latest_official",price,pct:resolveNavPct(fund,date),label:`最近官方净值 · ${date||"未知日期"}`,dataDate:date,confidence:"high",reason:"当前没有当日可验证盘中估值，明确标记为最近官方净值"}; }
export function selectFundDisplayQuote(fund: FundQuote|undefined,now=new Date()):FundDisplayQuote{
  ...
  if(isOfficialNavToday(fund,now))return{mode:"official_today",price:fund.nav,pct:resolveNavPct(fund,fund.navDate),label:"最近交易日官方净值",dataDate:fund.navDate,confidence:"high",reason:"数据层已确认最近交易日官方净值；收盘后非交易日继续沿用此净值"};
  ...
  if(getMarketPhase(now)==="postclose")return (fund.referenceNav??fund.nav)!=null&&((fund.referenceNavDate??fund.navDate)!=null)?latestOfficial(fund):{mode:"unavailable",price:null,pct:null,label:"等待今日数据",dataDate:null,confidence:"none",reason:"收盘后既没有当日可靠估值也没有可用官方净值"};
