...
export async function getLiveFundQuote(code: string): Promise<LiveFundQuote | null> {
  const urls: [string, string][] = [
    [`https://fundcomapi.tiantianfunds.com/mm/newCore/FundValuationLast?FCODES=${encodeURIComponent(code)}&FIELDS=FCODE,SHORTNAME,GSZZL,GZTIME,GSZ,NAV,PDATE&_=${Date.now()}`, "天天基金/东方财富实时估值"],
  ...
  const results = await Promise.all(urls.map(([url, source]) => tryJson(url, code, source)));
  const valid = results.filter((x): x is LiveFundQuote => !!x && (x.estimate != null || x.pct != null));
  if (!valid.length) return null;
  return valid.find((x) => x.estimate != null && x.pct != null) ?? valid[0];
}
