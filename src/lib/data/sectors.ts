...
export const SECTOR_RULES: SectorRule[] = [
  ...
  { id: "comm", name: "通信", bkCode: "BK1650", prefer: "industry", searchKeys: ["通信", "通信设备"], keys: ["通信设备", "通信", "5G"], etf: { code: "515880", name: "通信ETF" } },
...
export function matchFundSector(fundName: string): SectorRule | null {
  ...
  if (!n) return null;
  const ordered = [...SECTOR_RULES].sort((a, b) => Math.max(...b.keys.map((k) => k.length)) - Math.max(...a.keys.map((k) => k.length)));
  for (const rule of ordered) {
    if (rule.keys.some((key) => key.length >= 2 && n.includes(key))) return rule;
  }
  return null;
}
