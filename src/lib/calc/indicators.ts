...
export function calcIndicators(c: number[]): FundMetrics | null {
  ...
  const ma = (p: number) => c.length < p ? null : c.slice(-p).reduce((a, b) => a + b, 0) / p;
  const ma5 = ma(5);
  const ma10 = ma(10);
  const ma20 = ma(20);
  ...
  const upper = m + 2 * sd;
  const lower = m - 2 * sd;
  const bias = ma20 != null ? ((last - ma20) / ma20) * 100 : 0;
...
export function calcSwingTrade(metrics: FundMetrics | null, cost: number, price: number): SwingAdvice | null {
  ...
  const amplitude = upper > 0 && lower > 0 ? ((upper - lower) / ma20) * 100 : Math.abs(bias) + 3;
  ...
  if (env >= 70) { envLevel = "优质"; envText = "震荡充分，做T窗口好"; }
  else if (env >= 55) { envLevel = "一般"; envText = "可少量做T"; }
  if (!allowT) { envLevel = "不宜做T"; envText = trend > 78 ? "趋势行情禁止做T，以免卖飞" : "趋势偏弱，禁止做T以免接刀"; }
  const buyGrid = p * (1 - step / 100);
  const sellGrid = p * (1 + step / 100);
  let action = "观望";
  let reason = "信号不明确";
  if (!allowT) { action = "持有观察"; reason = envText; }
  else if (rsi < 32 || (lower > 0 && p <= lower) || bias < -6) { action = "低吸观察"; reason = "超卖/触及下轨，等待收盘确认后再考虑加仓"; }
  else if (rsi > 68 || (upper > 0 && p >= upper) || bias > 6) { action = "减仓提示"; reason = "超买/触及上轨，震荡市可高抛一部分"; }
  else if (gainPct > step * 1.2) { action = "卖出提示"; reason = `相对成本已有 ${gainPct.toFixed(1)}% 浮盈，可按网格落袋一部分`; }
  else { action = "网格等待"; reason = `步长约 ${step.toFixed(1)}%，高抛 ${sellGrid.toFixed(4)} / 低吸 ${buyGrid.toFixed(4)}`; }
