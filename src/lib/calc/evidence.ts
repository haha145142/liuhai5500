...
export function buildEvidence(snap: Snapshot, news: NewsItem[]): EvidenceResult {
  ...
  if (vals.length) step2 = strong.length >= 2 ? `市场有明显反应，${strong.length} 个板块涨幅与资金流向同时为正：${strong.map((x) => `${x.name} ${fmtPctShort(x.change)}`).join(" · ")}` : weak.length >= 2 ? `市场反应偏冷，${weak.length} 个板块跌幅与资金流向同时为负：${weak.map((x) => `${x.name} ${fmtPctShort(x.change)}`).join(" · ")}` : "市场反应平淡，涨跌分化，暂无明确主线。`;
  ...
  if (posNews > negNews * 2) bull += 1; if (negNews > posNews * 2) bear += 1;
  let verdict = "中性震荡"; let duration = "方向不明"; let confidence = "一般";
  if (weekend) { verdict = "周末观望"; duration = "下周一确认"; confidence = "周末模式"; }
  else if (bull - bear >= 3) { verdict = "偏利好"; duration = "短期可持续"; confidence = "较高"; }
  else if (bull - bear >= 1) { verdict = "温和偏多"; duration = "短期观望"; }
  else if (bear - bull >= 3) { verdict = "偏利空"; duration = "中期谨慎"; confidence = "较高"; }
  else if (bear - bull >= 1) { verdict = "温和偏空"; duration = "短期观望"; }
  const step7 = weekend ? "现在是周末，A股不开盘。以上市场数字沿用最近完成交易日数据，周一开盘后重新验证。" : `综合判断：${verdict}。持续性：${duration}。置信度：${confidence}。${strong.length ? `关注：${strong.map((x) => x.name).join("、")}。` : ""}${weak.length ? `回避：${weak.map((x) => x.name).join("、")}。` : ""}仅供参考，不构成投资建议。`;
  const score = weekend ? 50 : Math.max(15, Math.min(85, 50 + (bull - bear) * 8));
  const risk = Math.abs(avg ?? 0) > 1.2 ? "中高" : "中等";
  const summary = weekend ? "周末休市，今天不做实时仓位判断，市场数据沿用最近交易日。" : `今日大盘整体${(avg ?? 0) > 0.15 ? "偏暖" : (avg ?? 0) < -0.15 ? "偏冷" : "震荡"}，${verdict}。`;
  return { steps: [
...
/**
 * Historical implementation kept only for compatibility. It is deliberately no longer used
 * by the home UI because counting words in news cannot establish institutional/retail trading.
 */
