...
export function calibrateValuation(observations: ValuationObservation[]): CalibrationStats {
  const valid = observations.filter(
    (x) => Number.isFinite(x.estimatePct) && Number.isFinite(x.officialPct),
  );
  ...
  const agreement = valid
    .map((x) => x.sourceAgreementPct)
    .filter((x): x is number => Number.isFinite(x as number));

  const byType: CalibrationStats["byType"] = {};
  for (const item of valid) {
    const type = item.fundType || "未知类型";
    const bucket = byType[type] || { errors: [], directions: 0, total: 0 };
...
export function calibratedReliability(
