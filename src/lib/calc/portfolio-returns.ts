...
function selectReturnQuote(fund:FundQuote|undefined):ReturnQuote{
  ...
  // 当日官方净值未发布时，无论盘中还是收盘后，只要有当天估值就继续使用当天估值。
  if(current!=null&&(fund.valuationStatus==="estimate"||fund.valuationStatus==="live_estimate")&&fund.officialNavPublished!==true&&sameChinaDate(fund.estimateTime))return{price:current,mode:"live_estimate"};
...
function previousOfficialNav(fund:FundQuote|undefined,currentPrice:number|null,mode:ReturnQuote["mode"]){
  ...
  // 估值模式的基准永远是最近一笔官方净值，即使估值恰好与官方净值数值相同。
