 ...
 * This is intentionally labeled as an estimate: a weighted-average cost can
 * represent multiple purchases and therefore cannot identify an exact date.
 */
export function inferHoldingEntryDate(points: FundHistoryPoint[] | null | undefined, cost: number, today = new Date()): HoldingEntryEstimate {
  ...
  const candidates = points.filter(validPoint).map((point) => ({
    point,
    distancePct: Math.abs(point.nav - cost) / cost * 100,
  ...
  if (Number.isNaN(entry.getTime()) || entry.getTime() > today.getTime()) {
    return { date: null, days: null, nav: best.point.nav, distancePct: best.distancePct, confidence: "none" };
  }
