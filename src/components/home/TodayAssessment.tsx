...
export function TodayAssessment() {
  ...
  const quantitative = useMemo(() => snapshot ? calcMarketScoreV2(snapshot, news?.items || []) : null, [news, snapshot]);
  const score = quantitative?.score ?? null;
  const scoreLabel = quantitative?.label ?? "等待数据";
  const scoreTone = score == null ? 0 : score - 50;
  const coverage = quantitative ? `${quantitative.confidence} · ${quantitative.factors.length}项量化因子` : "等待可靠行情";
  ...
  const action = actionFor(result?.verdict || quantitative?.label || "", result?.confidence || (quantitative?.confidence === "高" ? "较高" : quantitative?.confidence === "中" ? "一般" : "低"), result?.verdict === "周末观望");
    ...
    <section className="mt-3" aria-label="今日评估">
      <Glass className="overflow-hidden rounded-[24px] border border-white/75 bg-white/54 p-3 shadow-[0_16px_44px_rgba(38,78,112,.07),inset_0_1px_0_rgba(255,255,255,.96)] backdrop-blur-[22px] saturate-150">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0"><div className="flex items-center gap-2"><span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-blue-500/10 text-[16px]">📝</span><div><div className="text-[16px] font-semibold tracking-tight text-fg">今日评估</div><div className="mt-0.5 text-[10px] text-muted">量化因子评分 · AI只解释，不直接生成分数</div></div></div></div>
          <DataStatus mode={quantitative?.confidence === "高" ? "live" : "latest"} detail={snapshot?.marketDate || undefined} />
        </div>
...
function Signal({ label, value }: { label: string; value: string }) { return <div className="rounded-[14px] bg-white/54 px-2.5 py-2 ring-1 ring-white/70"><div className="text-[9px] text-subtle">{label}</div><div className="mt-0.5 text-[11px] font-semibold text-fg">{value}</div></div>; }
