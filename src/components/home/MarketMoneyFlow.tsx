...
function isSnapshot(data: MarketMoneyFlow) { return data.super == null && data.large == null && data.mid == null && data.small == null; }
      ...
      <Glass tight className="overflow-hidden rounded-[24px] border border-white/75 bg-white/52 p-3 shadow-[0_14px_38px_rgba(38,78,112,.07)] backdrop-blur-[22px]">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-0.5 text-[10px] text-slate-500">{data && isSnapshot(data) ? "板块主力净流入口径 · 不推导订单分层" : "订单规模口径 · 观察资金进出与市场放缩量"}</div>
          </div>
        ...
        {data ? (
          <>
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <Metric label="样本覆盖" value={`${data.count.toLocaleString()}项`} tone="text-slate-800" />
            </div>
            ...
            <div className="mt-2 rounded-[15px] bg-white/58 px-3 py-2.5 text-[10px] leading-[1.55] text-slate-500">
              <b className="text-slate-800">口径：</b>{isSnapshot(data)
                ? "当前为后台采集的板块主力净流入快照，只用于观察板块资金方向；超大单、大单、中单、小单及全市场成交额没有可靠快照时不显示，不用其他口径补位。"
                : "超大单 + 大单只作为大资金/主力订单规模代理；中单、小单用于观察小额订单行为，不代表真实机构或散户身份。当前资金流来自单一供应商，已做内部结构平衡校验，因此不标记为双源确认。"}
            </div>
            <div className="mt-1.5 flex flex-wrap items-center justify-between gap-2 text-[9px] text-slate-400">
              ...
              <span>{data.validation === "fully_consistent" ? "结构一致" : data.validation === "partially_consistent" ? "板块资金快照" : "不可用于方向判断"}</span>
            </div>
          </>
        ) : (
