...
export function HomeDashboardV2() {
  ...
  const validation = snapshot?.validation === "cross_checked"
    ? "双源核验"
  ...
  return (
    <div className="home-dashboard-v3 pb-4">
      <Glass tight className="rounded-[28px] bg-white/58 p-4 shadow-[0_18px_50px_rgba(38,78,112,.09),inset_0_1px_0_rgba(255,255,255,.96)] backdrop-blur-[28px]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[11px] text-slate-500"><span>我的基金收益</span><Bell className="size-3.5" /></div>
            <div className="mt-1 text-[30px] font-bold tracking-tight text-slate-950 tabular-nums">{selected.amount == null ? "—" : fmtMoney(selected.amount)}</div>
            <div className="mt-1 text-[10px] text-slate-400">{selected.label} · {portfolio.length ? `${portfolio.length}只持仓` : "尚未建仓"}</div>
          </div>
        ...
        <div className="mt-3 grid grid-cols-5 gap-1 rounded-2xl bg-white/48 p-0.5 ring-1 ring-white/75">
          {(["today", "week", "month", "quarter", "since"] as const).map((tab) => {
            const label = tab === "today" ? "今天" : tab === "week" ? "一周" : tab === "month" ? "一个月" : tab === "quarter" ? "近3月" : "买入以来";
            return <button key={tab} type="button" onClick={() => setPeriodTab(tab)} className={`rounded-[14px] px-1.5 py-2 text-[11px] font-medium transition ${periodTab === tab ? "bg-blue-500 text-white shadow-[0_4px_12px_rgba(59,130,246,.20)]" : "bg-white/66 text-muted"}`}>{label}</button>;
          })}
        </div>
        <div className="mt-2 grid grid-cols-2 gap-2">
          <Metric label="持仓市值" value={summary?.marketValue == null ? "—" : fmtMoney(summary.marketValue)} tone={summary?.marketValue ?? null} />
      ...
      <section className="mt-3 grid grid-cols-4 gap-2" aria-label="常用功能">
        {QUICK.map(([to, label, icon]) => <Link key={to} to={to} className="flex min-h-[66px] flex-col items-center justify-center rounded-[20px] border border-white/75 bg-white/56 px-1.5 shadow-[0_8px_24px_rgba(38,78,112,.06)] backdrop-blur-[22px] active:scale-[0.98]" aria-label={label}><span className="flex size-9 items-center justify-center rounded-full bg-white/78 text-[18px] shadow-sm">{icon}</span><span className="mt-1.5 text-[10px] font-semibold text-slate-700">{label}</span></Link>)}
      </section>
