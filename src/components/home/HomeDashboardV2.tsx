...
import { MarketMoneyFlow } from "@/components/home/MarketMoneyFlow";
...
export function HomeDashboardV2() {
  ...
  const periods = useMemo(() => ({ week: calcPortfolioPeriodReturn("week", portfolio, funds), month: calcPortfolioPeriodReturn("month", portfolio, funds), quarter: calcPortfolioPeriodReturn("quarter", portfolio, funds) }), [funds, portfolio]);
  const selected = useMemo(() => {
    ...
    if (periodTab === "since") return { label: fullyPriced ? "买入以来收益" : "买入以来收益（已计价）", amount: summary.holdingPnl, pct: summary.pricedHoldingPnlPct };
    ...
    <section className="home-income-card" aria-label="持仓总收益">
      <Glass tight className="mt-0 rounded-[28px] bg-white/58 p-4 shadow-[0_18px_50px_rgba(38,78,112,.09),inset_0_1px_0_rgba(255,255,255,.96)] backdrop-blur-[28px]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0"><div className="text-[11px] font-medium text-slate-500">{fullyPriced ? "持仓总收益" : "持仓总收益（已计价部分）"}</div><div className="mt-1 text-[30px] font-bold tracking-tight text-slate-950 tabular-nums">{summary?.holdingPnl == null ? "—" : fmtMoney(summary.holdingPnl)}</div><div className="mt-1 text-[10px] text-slate-400">当前全部持仓 · {portfolio.length ? `${portfolio.length}只基金` : "尚未建仓"}</div></div>
          <span className="shrink-0 rounded-full bg-white/72 px-2.5 py-1 text-[9px] font-semibold text-slate-500">{summary?.pricedHoldingPnlPct == null ? "—" : fmtPctShort(summary.pricedHoldingPnlPct)}</span>
        </div>
        <div className="home-period-switch mt-3 grid grid-cols-5 gap-1 rounded-2xl bg-white/48 p-0.5 ring-1 ring-white/75">{(["today", "week", "month", "quarter", "since"] as const).map((tab) => { const label = tab === "today" ? "今天" : tab === "week" ? "一周" : tab === "month" ? "一个月" : tab === "quarter" ? "近3月" : "买入以来"; return <button key={tab} type="button" onClick={() => setPeriodTab(tab)} className={`home-period-btn rounded-[14px] px-1.5 py-2 text-[12px] font-medium transition ${periodTab === tab ? "is-active bg-blue-500 text-white shadow-[0_4px_12px_rgba(59,130,246,.20)]" : "bg-white/66 text-muted"}`}>{label}</button>; })}</div>
        <div className="mt-2 rounded-[18px] bg-white/54 px-3 py-2.5 ring-1 ring-white/75"><div className="flex items-center justify-between gap-3"><div><div className="text-[10px] text-slate-400">{selected.label}</div><Tone v={selected.amount} className="mt-1 block text-[17px] font-bold tabular-nums">{selected.amount == null ? "—" : fmtMoney(selected.amount)}</Tone></div><Tone v={selected.pct} className="text-[11px] font-semibold tabular-nums">{selected.pct == null ? "—" : fmtPctShort(selected.pct)}</Tone></div></div>
        <div className="mt-2 grid grid-cols-2 gap-2"><Metric label="持仓市值" value={summary?.marketValue == null ? "—" : fmtMoney(summary.marketValue)} tone={summary?.marketValue ?? null} /><Metric label="持有收益率" value={summary?.pricedHoldingPnlPct == null ? "—" : fmtPctShort(summary.pricedHoldingPnlPct)} tone={summary?.pricedHoldingPnlPct ?? null} /></div>
        <div className="mt-3 flex items-center justify-between border-t border-white/75 pt-2.5 text-[10px] text-slate-400"><span>{snapshot?.marketDate || "等待行情日期"}</span><span>{validation}</span></div>
        {summary && !fullyPriced ? <div className="mt-2 rounded-xl bg-amber-50/70 px-3 py-1.5 text-[10px] leading-relaxed text-slate-500">还有 {summary.totalCount - summary.pricedCount} 只持仓缺少可靠计价；上面的金额与收益率只代表已计价部分，不用不可靠数字补齐。</div> : null}
        {periodTab !== "today" && selected.amount == null ? <div className="mt-2 rounded-xl bg-amber-50/70 px-3 py-1.5 text-[10px] leading-relaxed text-slate-500">该周期暂缺足够历史净值，无法可靠计算时保持空白。</div> : null}
      </Glass>
    </section>
...
function Metric({ label, value, tone }: { label: string; value: string; tone: number | null }) {
  return <div className="rounded-[18px] bg-white/54 px-3 py-2.5 ring-1 ring-white/75"><div className="text-[10px] text-slate-400">{label}</div><Tone v={tone} className="mt-1 block text-[16px] font-bold tabular-nums">{value}</Tone></div>;
}
