  ...
  return <div className="home-dashboard-v3 pb-4">
    <section className="home-income-card" aria-label="持仓总收益">
      <Glass tight className="mt-0 rounded-[28px] bg-white/58 p-4 shadow-[0_18px_50px_rgba(38,78,112,.09),inset_0_1px_0_rgba(255,255,255,.96)] backdrop-blur-[28px]">
        ...
        {!fullyPriced && summary ? <div className="mt-2 rounded-xl bg-amber-50/70 px-3 py-1.5 text-[10px] leading-relaxed text-slate-500">还有 {summary.totalCount - summary.pricedCount} 只持仓缺少可靠计价，组合总收益暂不显示；数据不完整时不猜。</div> : null}
        {periodTab !== "since" && selected.amount == null ? <div className="mt-2 rounded-xl bg-amber-50/70 px-3 py-1.5 text-[10px] leading-relaxed text-slate-500">该周期需要全部持仓具备对应历史净值；数据不完整时不猜收益。</div> : null}
      </Glass>
    </section>
    ...
    {latestNews.length ? <section className="home-news-section mb-3 overflow-hidden rounded-[24px] border border-white/75 bg-white/48 p-3 shadow-[0_14px_38px_rgba(38,78,112,.07)] backdrop-blur-[20px]" aria-label="最新资讯"><div className="mb-2 flex items-center justify-between"><div><div className="text-[17px] font-semibold tracking-tight text-fg">最新资讯</div><div className="text-[10px] text-muted">只显示最新 5 条</div></div><Link to="/news" className="text-[10px] text-blue-600">全部 →</Link></div><div className="space-y-1.5">{latestNews.map((item) => <a key={`${item.source}-${item.publishedAt}-${item.title}`} href={item.url || "#"} target="_blank" rel="noreferrer" className="block rounded-[14px] bg-white/58 px-2.5 py-2.5 active:bg-white/75"><div className="line-clamp-2 text-[11px] font-medium text-slate-800">{item.title}</div><div className="mt-1 text-[9px] text-slate-400">{item.source || "新闻"} · {item.publishedAt || "时间未知"}</div></a>)}</div></section> : null}
