...
const QUICK = [["/portfolio", "添加基金", "＋"], ["/band", "波段信号", "🌈"], ["/market", "行情中心", "📊"], ["/funds", "基金排行", "🏆"]] as const;
...
export function HomeDashboardV2() {
  ...
  return <div className="home-dashboard-v3 pb-4">
    ...
    <Glass tight className="mt-3 rounded-[28px] bg-white/58 p-4 shadow-[0_18px_50px_rgba(38,78,112,.09),inset_0_1px_0_rgba(255,255,255,.96)] backdrop-blur-[28px]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0"><div className="flex items-center gap-2 text-[11px] text-slate-500"><span>我的基金收益</span><Bell className="size-3.5" /></div><div className="mt-1 text-[30px] font-bold tracking-tight text-slate-950 tabular-nums">{selected.amount == null ? "—" : fmtMoney(selected.amount)}</div><div className="mt-1 text-[10px] text-slate-400">{selected.label} · {portfolio.length ? `${portfolio.length}只持仓` : "尚未建仓"}</div></div>
        <span className="shrink-0 rounded-full bg-white/72 px-2.5 py-1 text-[9px] text-slate-500">{selected.pct == null ? "—" : fmtPctShort(selected.pct)}</span>
      </div>
      <div className="mt-3 grid grid-cols-5 gap-1 rounded-2xl bg-white/48 p-0.5 ring-1 ring-white/75">{(["today", "week", "month", "quarter", "since"] as const).map((tab) => { const label = tab === "today" ? "今天" : tab === "week" ? "一周" : tab === "month" ? "一个月" : tab === "quarter" ? "近3月" : "买入以来"; return <button key={tab} type="button" onClick={() => setPeriodTab(tab)} className={`rounded-[14px] px-1.5 py-2 text-[11px] font-medium transition ${periodTab === tab ? "bg-blue-500 text-white shadow-[0_4px_12px_rgba(59,130,246,.20)]" : "bg-white/66 text-muted"}`}>{label}</button>; })}</div>
    ...
    <section className="mt-3 grid grid-cols-4 gap-2" aria-label="常用功能">{QUICK.map(([to, label, icon]) => <Link key={to} to={to} className="flex min-h-[66px] flex-col items-center justify-center rounded-[20px] border border-white/75 bg-white/56 px-1.5 shadow-[0_8px_24px_rgba(38,78,112,.06)] backdrop-blur-[22px] active:scale-[0.98]" aria-label={label}><span className="flex size-9 items-center justify-center rounded-full bg-white/78 text-[18px] shadow-sm">{icon}</span><span className="mt-1.5 text-[10px] font-semibold text-slate-700">{label}</span></Link>)}</section>

    <section className="mt-4" aria-label="我的基金"><div className="mb-2 flex items-end justify-between"><div><div className="text-[16px] font-semibold tracking-tight text-slate-950">我的基金</div><div className="text-[10px] text-slate-400">实时估值 · 持有收益 · 紧凑列表</div></div><Link to="/portfolio" className="text-[10px] font-medium text-blue-600">全部 {portfolio.length || 0} 只</Link></div>{portfolio.length ? <Glass tight className="overflow-hidden p-0"><div className="divide-y divide-white/70">{visible.map((h) => { const f = funds[h.code]; const r = f ? calcHoldingReturn(h, f) : null; const priceLabel = r?.quoteMode === "live_estimate" ? "估值" : "净值"; const price = r?.price == null ? "—" : r.price.toFixed(4); return <Link key={h.code} to="/portfolio" className="flex min-h-[58px] items-center gap-2.5 px-3.5 py-2 active:bg-white/45"><span className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-white/72 text-sm shadow-sm">📈</span><span className="min-w-0 flex-1"><span className="block truncate text-[12px] font-semibold text-slate-900">{f?.name || h.name}</span><span className="mt-0.5 block truncate text-[9px] text-slate-400">{h.code} · {priceLabel} {price}</span></span><span className="shrink-0 text-right"><Tone v={r?.todayPnlPct ?? null} className="block text-[14px] font-bold tabular-nums">{r?.todayPnlPct == null ? "—" : fmtPctShort(r.todayPnlPct)}</Tone><Tone v={r?.holdingPnlPct ?? null} className="mt-0.5 block text-[9px] font-medium tabular-nums">持有 {r?.holdingPnlPct == null ? "—" : fmtPctShort(r.holdingPnlPct)}</Tone></span><ChevronRight className="size-3.5 shrink-0 text-slate-300" /></Link>; })}</div>{portfolio.length > 8 ? <button type="button" onClick={() => setExpanded((v) => !v)} className="flex w-full items-center justify-center border-t border-white/70 py-2 text-[10px] font-medium text-slate-500">{expanded ? "收起" : `展开剩余 ${portfolio.length - 8} 只`}</button> : null}</Glass> : <Glass tight className="border-dashed text-center"><div className="py-3 text-[11px] text-slate-500">还没有持仓</div><Link to="/portfolio" className="inline-flex items-center gap-1 rounded-full bg-slate-900 px-3 py-1.5 text-[10px] text-white"><Plus className="size-3" />添加基金</Link></Glass>}</section>

    <section className="mt-3" aria-label="自选板块"><FundSectorWatchV2 /></section>
    <OperationAdvice sectors={snapshot?.sectors || []} benchPct={benchPct} />

    {latestNews.length ? <section className="mb-3 overflow-hidden rounded-[24px] border border-white/75 bg-white/48 p-3 shadow-[0_14px_38px_rgba(38,78,112,.07)] backdrop-blur-[20px]" aria-label="最新资讯"><div className="mb-2 flex items-center justify-between"><div><div className="text-[16px] font-semibold tracking-tight text-fg">📰 最新资讯</div><div className="text-[9px] text-muted">只显示最新 5 条，不再重复生成市场解读</div></div><Link to="/news" className="text-[9px] text-blue-600">全部 →</Link></div><div className="space-y-1.5">{latestNews.map((item) => <a key={`${item.source}-${item.publishedAt}-${item.title}`} href={item.url || "#"} target="_blank" rel="noreferrer" className="block rounded-[14px] bg-white/58 px-2.5 py-2 active:bg-white/75"><div className="line-clamp-2 text-[10px] font-medium text-slate-800">{item.title}</div><div className="mt-1 text-[8px] text-slate-400">{item.source || "新闻"} · {item.publishedAt || "时间未知"}</div></a>)}</div></section> : null}

    <button type="button" onClick={() => setServicesOpen(true)} className="mt-3 flex w-full items-center justify-between rounded-[21px] border border-white/75 bg-white/58 px-4 py-3 shadow-[0_10px_30px_rgba(38,78,112,.07)] backdrop-blur-[24px]" aria-label="打开全部服务"><span className="flex items-center gap-2.5"><span className="flex size-9 items-center justify-center rounded-full bg-white/80 shadow-sm"><Sparkles className="size-4 text-slate-700" /></span><span className="text-left"><span className="block text-[13px] font-semibold text-slate-900">全部服务</span><span className="block text-[9px] text-slate-400">低频功能集中收纳，不占首页空间</span></span></span><ChevronRight className="size-4 text-slate-400" /></button>
