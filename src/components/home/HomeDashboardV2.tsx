...
import { Bell, ChevronRight, Plus, Sparkles, X } from "lucide-react";
...
export function HomeDashboardV2() {
  ...
  return <div className="home-dashboard-v3 pb-4">
    <Glass tight className="home-income-card mt-3 rounded-[28px] bg-white/58 p-4 shadow-[0_18px_50px_rgba(38,78,112,.09),inset_0_1px_0_rgba(255,255,255,.96)] backdrop-blur-[28px]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0"><div className="flex items-center gap-2 text-[11px] text-slate-500"><span>总收益</span><Bell className="size-3.5" /></div><div className="mt-1 text-[30px] font-bold tracking-tight text-slate-950 tabular-nums">{selected.amount == null ? "—" : fmtMoney(selected.amount)}</div><div className="mt-1 text-[10px] text-slate-400">{selected.label} · {portfolio.length ? `${portfolio.length}只持仓` : "尚未建仓"}</div></div>
        <span className="home-income-rate shrink-0 rounded-full bg-white/72 px-2.5 py-1 text-[9px] text-slate-500">{selected.pct == null ? "—" : fmtPctShort(selected.pct)}</span>
      </div>
      ...
      {periodTab !== "today" && selected.amount == null ? <div className="mt-2 rounded-xl bg-amber-50/70 px-3 py-1.5 text-[9px] leading-relaxed text-slate-500">该周期需要全部持仓具备对应历史净值；数据不完整时不猜收益。</div> : null}
    </Glass>

    <TodayAssessment />

    <section className="home-portfolio-section mt-4" aria-label="我的基金"><div className="home-section-head mb-2 flex items-end justify-between"><div><div className="text-[16px] font-semibold tracking-tight text-slate-950">我的持仓</div><div className="text-[10px] text-slate-400">实时估值 · 持有收益 · 组合体检 · 波段 / 趋势 / 做T</div></div><Link to="/portfolio" className="text-[10px] font-medium text-blue-600">全部 {portfolio.length || 0} 只</Link></div><div className="home-portfolio-tools mb-2 grid grid-cols-2 gap-2"><Link to="/portfolio" className="home-tool-pill group flex items-center justify-between rounded-[16px] bg-white/54 px-3 py-2.5 ring-1 ring-white/75"><span><span className="block text-[10px] font-semibold text-slate-700">组合体检</span><span className="mt-0.5 block text-[8px] text-slate-400">集中看健康度与风险</span></span><ChevronRight className="size-3.5 text-slate-300 transition group-active:translate-x-0.5" /></Link><Link to="/band" className="home-tool-pill group flex items-center justify-between rounded-[16px] bg-white/54 px-3 py-2.5 ring-1 ring-white/75"><span><span className="block text-[10px] font-semibold text-slate-700">波段 / 做T</span><span className="mt-0.5 block text-[8px] text-slate-400">看趋势、位置和今日动作</span></span><ChevronRight className="size-3.5 text-slate-300 transition group-active:translate-x-0.5" /></Link></div>{portfolio.length ? <Glass tight className="home-portfolio-card overflow-hidden p-0"><div className="divide-y divide-white/70">{visible.map((h) => { const f = funds[h.code]; const r = f ? calcHoldingReturn(h, f) : null; const priceLabel = r?.quoteMode === "live_estimate" ? "估值" : "净值"; const price = r?.price == null ? "—" : r.price.toFixed(4); return <Link key={h.code} to="/portfolio" className="home-fund-row flex min-h-[58px] items-center gap-2.5 px-3.5 py-2 active:bg-white/45"><span className="home-fund-icon flex size-8 shrink-0 items-center justify-center rounded-xl bg-white/72 text-sm shadow-sm">📈</span><span className="min-w-0 flex-1"><span className="block truncate text-[12px] font-semibold text-slate-900">{f?.name || h.name}</span><span className="mt-0.5 block truncate text-[9px] text-slate-400">{h.code} · {priceLabel} {price}</span></span><span className="shrink-0 text-right"><Tone v={r?.todayPnlPct ?? null} className="block text-[14px] font-bold tabular-nums">{r?.todayPnlPct == null ? "—" : fmtPctShort(r.todayPnlPct)}</Tone><Tone v={r?.holdingPnlPct ?? null} className="mt-0.5 block text-[9px] font-medium tabular-nums">持有 {r?.holdingPnlPct == null ? "—" : fmtPctShort(r.holdingPnlPct)}</Tone></span><ChevronRight className="size-3.5 shrink-0 text-slate-300 transition-transform" /></Link>; })}</div>{portfolio.length > 8 ? <button type="button" onClick={() => setExpanded((v) => !v)} className="home-expand-btn flex w-full items-center justify-center border-t border-white/70 py-2 text-[10px] font-medium text-slate-500">{expanded ? "收起" : `展开剩余 ${portfolio.length - 8} 只`}</button> : null}</Glass> : <Glass tight className="border-dashed text-center"><div className="py-3 text-[11px] text-slate-500">还没有持仓</div><Link to="/portfolio" className="inline-flex items-center gap-1 rounded-full bg-slate-900 px-3 py-1.5 text-[10px] text-white shadow-[0_8px_20px_rgba(15,23,42,.15)] active:scale-[0.98]"><Plus className="size-3" />添加基金</Link></Glass>}</section>
    ...
    {latestNews.length ? <section className="home-news-section mb-3 overflow-hidden rounded-[24px] border border-white/75 bg-white/48 p-3 shadow-[0_14px_38px_rgba(38,78,112,.07)] backdrop-blur-[20px]" aria-label="最新资讯"><div className="mb-2 flex items-center justify-between"><div><div className="text-[16px] font-semibold tracking-tight text-fg">最新资讯</div><div className="text-[9px] text-muted">只保留最新 5 条</div></div><Link to="/news" className="text-[9px] text-blue-600">全部 →</Link></div><div className="space-y-1.5">{latestNews.map((item) => <a key={`${item.source}-${item.publishedAt}-${item.title}`} href={item.url || "#"} target="_blank" rel="noreferrer" className="home-news-item block rounded-[14px] bg-white/58 px-2.5 py-2 active:bg-white/75"><div className="line-clamp-2 text-[10px] font-medium text-slate-800">{item.title}</div><div className="mt-1 text-[8px] text-slate-400">{item.source || "新闻"} · {item.publishedAt || "时间未知"}</div></a>)}</div></section> : null}

    <button type="button" onClick={() => setServicesOpen(true)} className="home-services-trigger mt-3 flex w-full items-center justify-between rounded-[21px] border border-white/75 bg-white/58 px-4 py-3 shadow-[0_10px_30px_rgba(38,78,112,.07)] backdrop-blur-[24px]" aria-label="打开全部服务"><span className="flex items-center gap-2.5"><span className="flex size-9 items-center justify-center rounded-full bg-white/80 shadow-sm"><Sparkles className="size-4 text-slate-700" /></span><span className="text-left"><span className="block text-[13px] font-semibold text-slate-900">全部服务</span><span className="block text-[9px] text-slate-400">低频功能集中收纳，不占首页空间</span></span></span><ChevronRight className="size-4 text-slate-400 transition-transform" /></button>
    {servicesOpen ? <div className="home-services-backdrop fixed inset-0 z-[7000] flex items-end bg-slate-950/20 backdrop-blur-[4px]" role="dialog" aria-modal="true" aria-label="全部服务" onClick={() => setServicesOpen(false)}><div className="home-services-sheet w-full rounded-t-[30px] bg-white/82 px-4 pb-[max(20px,env(safe-area-inset-bottom))] pt-4 shadow-[0_-20px_70px_rgba(22,42,64,.20)] backdrop-blur-[32px]" onClick={(e) => e.stopPropagation()}><div className="mx-auto mb-3 h-1 w-10 rounded-full bg-slate-300/70" /><div className="flex items-center justify-between"><div><div className="text-[18px] font-semibold text-slate-950">全部服务</div><div className="text-[10px] text-slate-400">只收纳当前已经存在的功能</div></div><button type="button" onClick={() => setServicesOpen(false)} aria-label="关闭全部服务" className="flex size-9 items-center justify-center rounded-full bg-white/72 active:scale-[0.96]"><X className="size-4" /></button></div><div className="mt-4 grid grid-cols-3 gap-2.5">{SERVICES.map(([to, title, icon]) => <Link key={`${to}-${title}`} to={to} onClick={() => setServicesOpen(false)} className="home-service-item rounded-[18px] bg-white/62 px-2 py-3 text-center ring-1 ring-white/80 active:scale-[0.98]"><span className="mx-auto flex size-10 items-center justify-center rounded-[14px] bg-white/78 text-lg shadow-sm">{icon}</span><span className="mt-1.5 block truncate text-[10px] font-semibold text-slate-800">{title}</span></Link>)}</div></div></div> : null}
  </div>;
}
