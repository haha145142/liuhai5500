    ...
    <section className="home-income-card" aria-label="总收益">
      <Glass tight className="mt-0 rounded-[28px] bg-white/58 p-4 shadow-[0_18px_50px_rgba(38,78,112,.09),inset_0_1px_0_rgba(255,255,255,.96)] backdrop-blur-[28px]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0"><div className="text-[11px] font-medium text-slate-500">总收益</div><div className="mt-1 text-[30px] font-bold tracking-tight text-slate-950 tabular-nums">{selected.amount == null ? "—" : fmtMoney(selected.amount)}</div><div className="mt-1 text-[10px] text-slate-400">{selected.label} · {portfolio.length ? `${portfolio.length}只持仓` : "尚未建仓"}</div></div>
          <span className="shrink-0 rounded-full bg-white/72 px-2.5 py-1 text-[9px] font-semibold text-slate-500">{selected.pct == null ? "—" : fmtPctShort(selected.pct)}</span>
        </div>
