...
import { FundIntelligenceStrip } from "@/components/portfolio/FundIntelligenceStrip";
...
function PortfolioPage() {
  ...
  const visiblePortfolio = useMemo(() => filterCodes ? portfolio.filter((h) => filterCodes.has(h.code)) : portfolio, [filterCodes, portfolio]);
  ...
  const calendarPnl = useMemo<CalendarPnl>(() => {
    ...
    for (const h of portfolio) {
      ...
      for (let i = 1; i < points.length; i += 1) {
        ...
        if (!current || !previous || current.date === previous.date || current.nav == null || previous.nav == null) continue;
  ...
  return (
    <div>
      ...
      <PortfolioGroups holdings={portfolio} funds={funds} onFilter={setFilterCodes} />
      <Glass className="mb-3 rounded-[28px] border border-white/75 bg-white/48 p-3 shadow-[0_18px_48px_rgba(38,78,112,.07)] backdrop-blur-[20px] saturate-150">
        {visiblePortfolio.length ? visiblePortfolio.map((h) => { const fname = funds[h.code]?.name || h.name; const rule = matchFundSector(fname); const sector = rule ? snapshot?.sectors.find((s) => s.id === rule.id) : undefined; return <div key={h.code}><FundCard holding={h} fund={funds[h.code]} sector={sector} benchPct={bench} totalMarketValue={summary.marketValue || 0} expandedOverride={bulkExpanded} onUpdate={(patch) => updateHolding(h.code, patch)} onRemove={() => removeHolding(h.code)} /><FundIntelligenceStrip holding={h} fund={funds[h.code]} sector={sector} benchPct={bench} /></div>; }) : <EmptyNote>当前组合没有基金。</EmptyNote>}
        <QuickAddFund />
      </Glass>
      <PortfolioInsight holdings={visiblePortfolio} funds={Object.values(funds)} sectors={snapshot?.sectors || []} />
      <Glass>
      ...
      <div className="mt-3"><SmartAlerts /></div>
    </div>
  );
}
function CalendarCell({ date, inMonth, calendarPnl }: { date: Date; inMonth: boolean; calendarPnl: CalendarPnl }) { const key = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`; const day = calendarPnl.get(key); const pnl = day?.pnl ?? 0; const found = day?.found === true; return <div className={`min-h-12 rounded-xl p-1 ${inMonth ? "bg-bg-elevated" : "opacity-30"}`}><div className="text-xs">{date.getDate()}</div><Tone v={found ? pnl : null} className="mt-1 block text-[10px] font-semibold">{found ? fmtMoney(pnl) : "—"}</Tone></div>; }
