...
export function FundSectorWatch() {
  ...
  return <section className="fsw-wrap">
    ...
    <div className="fsw-list">{rows.length ? rows.map((row)=>{ const open=expanded===row.id; const heldCodes=new Set(portfolio.map((h)=>h.code)); const valid=row.funds.filter((f)=>f.pct!=null); const leaders=valid.slice().sort((a,b)=>(b.pct??-Infinity)-(a.pct??-Infinity)); return <article key={row.id} className={`fsw-card ${open?"open":""}`}>
      <button type="button" className="fsw-main" onClick={()=>setExpanded(open?null:row.id)} aria-expanded={open}>
        <div className="fsw-main-head"><div className="fsw-name-block"><span className="fsw-icon">{row.icon}</span><div><div className="fsw-name">{row.name}<span className="fsw-fund-count">{row.validCount}/{row.totalCount} 只基金</span></div><div className="fsw-mini-source">{validationText(row)} · {row.marketDate || "日期未知"} · {row.source}</div></div></div><div className={`fsw-pct ${tone(row.pct)}`}>{pctText(row.pct)}</div></div>
      ...
      {open?<div className="fsw-funds"><div className="fsw-fund-head"><span>包含基金</span><span>价格</span><span>涨跌</span></div>{row.funds.map((fund)=>{const q=displayQuote(fund);return <div className="fsw-fund-row" key={fund.code}><div className="fsw-fund-info"><div className="fsw-fund-name">{fund.name}{heldCodes.has(fund.code)?<span className="held">持有</span>:null}</div><div className="fsw-fund-meta">{fund.code} · {fund.validation==="cross_checked"?"双源":fund.validation==="single_source"?"单源":"暂无"}{fund.time?` · ${fund.time}`:""}{fund.date?` · ${fund.date}`:""}</div></div><div className="fsw-fund-value">{q.value==null?"—":q.value.toFixed(4)}<small>{q.label}</small></div><div className={`fsw-fund-pct ${tone(fund.pct)}`}>{pctText(fund.pct)}</div></div>})}<div className="fsw-expanded-note">板块涨跌 = 有效成分基金涨跌幅等权平均；没有可靠数据的基金不参与计算，也不会被当成 0。</div></div>:null}
