...
export function FundSectorWatch() {
  ...
  const refresh = async (force = false) => {
    ...
    try {
      ...
      if (result.rows.some((r) => r.validCount > 0)) {
        setRows(result.rows); const date = result.rows.find((r) => r.marketDate)?.marketDate || tradingDateLabel(); saveCache(result.rows, date); setStale(result.weekend);
  ...
  return <section className="fsw-wrap">
    <div className="fsw-header">
      ...
      <div className="fsw-subtitle">基金主题板块 · 不看股票行业；板块涨跌 = 有效成分基金估值等权平均</div>
      <button type="button" className="fsw-manage" onClick={() => setManagerOpen(true)}>＋ 管理板块</button>
    </div>

    <div className="fsw-status"><span className={loading ? "dot pulse" : "dot"} /><span>{statusText}</span><span className="fsw-status-right">{loading ? "更新中" : stale ? "最近交易日" : "实时更新"}</span></div>

    <div className="fsw-list">
      {rows.length ? rows.map((row) => {
        const open = expanded === row.id; const heldCodes = new Set(portfolio.map((h) => h.code));
        return <article key={row.id} className={`fsw-card ${open ? "open" : ""}`}>
          ...
          {open ? <div className="fsw-funds">{row.funds.map((fund) => { const value = fund.estimate ?? fund.nav; return <div className="fsw-fund-row" key={fund.code}><div className="fsw-fund-info"><div className="fsw-fund-name">{fund.name}{heldCodes.has(fund.code) ? <span className="held">持有</span> : null}</div><div className="fsw-fund-meta">{fund.code} · {fund.validation === "cross_checked" ? "双源" : fund.validation === "single_source" ? "单源" : "暂无"}{fund.time ? ` · ${fund.time}` : ""}</div></div><div className="fsw-fund-value">{value == null ? "—" : value.toFixed(4)}</div><div className={`fsw-fund-pct ${tone(fund.pct)}`}>{pctText(fund.pct)}</div></div>; })}<div className="fsw-expanded-note">板块涨跌 = 有效成分基金估值涨跌幅等权平均；无可靠数据的基金不参与计算，也不会被当作 0。</div></div> : null}
        </article>;
      }) : <div className="fsw-empty"><div>📊</div><b>还没有可靠的基金板块数据</b><span>点击“管理板块”选择你真正关注的基金主题。</span></div>}
    </div>

    {managerOpen ? <div className="fsw-mask" onMouseDown={(e) => { if (e.target === e.currentTarget) setManagerOpen(false); }}><div className="fsw-sheet"><div className="fsw-sheet-handle" /><div className="fsw-sheet-title"><div><h3>管理基金板块</h3><p>你选择什么，这里就显示什么</p></div><button type="button" onClick={() => setManagerOpen(false)}>完成</button></div><input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} className="fsw-search" placeholder="搜索：半导体 / AI / 医药 / 黄金 / 白酒…" /><div className="fsw-sheet-count">已关注 {validSelected.length} 个 · 共 {FUND_SECTORS.length} 个基金主题</div><div className="fsw-manager-list">{manager.map((s) => <button key={s.id} type="button" className={`fsw-manager-row ${selectedSet.has(s.id) ? "selected" : ""}`} onClick={() => toggleSector(s.id)}><span className="mi">{s.icon}</span><span className="mn"><b>{s.name}</b><small>{s.funds.length} 只基金</small></span><span className="check">{selectedSet.has(s.id) ? "✓" : "+"}</span></button>)}</div></div></div> : null}
  </section>;
}
