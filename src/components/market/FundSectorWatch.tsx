...
export function FundSectorWatch() {
  ...
  return <section className="fsw-wrap">
    ...
    <div className="fsw-list">{rows.length ? rows.map((row)=>{ const open=expanded===row.id; const valid=row.funds.filter((f)=>f.pct!=null); const leaders=valid.slice().sort((a,b)=>(b.pct??-Infinity)-(a.pct??-Infinity)); const displayFunds=open?leaders:valid; const heldFunds=row.funds.filter((f)=>holdingMap.has(f.code)); const heldValue=heldFunds.reduce((sum,f)=>{ const h=holdingMap.get(f.code); if(!h) return sum; const q=displayQuote(f); const px=q.value; return sum+(px!=null?px*h.shares:h.cost*h.shares); },0); const impact=row.pct!=null&&heldValue>0?heldValue*row.pct/100:null; return <article key={row.id} className={`fsw-card ${open?"open":""}`}>
      <button type="button" className="fsw-main" onClick={()=>setExpanded(open?null:row.id)} aria-expanded={open}>
        ...
        <div className="fsw-trust-row"><span>{heldFunds.length ? `持仓 ${heldFunds.length} 只 · 暴露 ${money(heldValue)}` : "未持有该板块基金"}</span>{heldFunds.length ? <b>{impact == null ? "影响待数据" : `今日影响≈ ${money(impact)}`}</b> : <b>{sectorTrust(row)}</b>}</div>
      </button>
      {open?<div className="fsw-funds"><div className="fsw-fund-head"><span>包含基金</span><span>价格</span><span>涨跌</span></div>{displayFunds.map((fund)=>{const q=displayQuote(fund);return <div className="fsw-fund-row" key={fund.code}><div className="fsw-fund-info"><div className="fsw-fund-name">{fund.name}{holdingMap.has(fund.code)?<span className="held">持有</span>:null}</div><div className="fsw-fund-meta">{fund.code} · {fund.validation==="cross_checked"?"双源":fund.validation==="single_source"?"单源":"暂无"}{fund.time?` · ${fund.time}`:""}{fund.date?` · ${fund.date}`:""}</div></div><div className="fsw-fund-value">{q.value==null?"—":q.value.toFixed(4)}<small>{q.label}</small></div><div className={`fsw-fund-pct ${tone(fund.pct)}`}>{pctText(fund.pct)}</div></div>})}<div className="fsw-expanded-note">板块涨跌 = 有效成分基金涨跌幅等权平均；没有可靠数据的基金不参与计算，也不会被当成 0。展开后按涨跌从强到弱排序。持仓影响为板块内关联基金当前持仓市值 × 板块涨跌的便捷估算，不等同于组合实际净收益。</div></div>:null}
