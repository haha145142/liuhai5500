...
export function FundSectorWatch() {
  ...
  const statusText = stale ? `休市/缓存 · 数据日 ${marketDate || tradingDateLabel()}` : `${sessionLabel()} · 基金主题估值 + 股票板块资金`;
  ...
  return <section className="fsw-wrap">
    <div className="fsw-header">
      ...
      <div className="fsw-subtitle">基金组合负责回答“我手里这组基金现在大概怎样”，股票板块资金负责提供“对应市场资金正在怎样走”。两者分开展示，不混成一个数字。</div>
      ...
      {rows.length ? rows.map((row) => {
        ...
        return <article key={row.id} className={`fsw-card ${open ? "open" : ""}`}>
          <button type="button" className="fsw-main" onClick={() => setExpanded(open ? null : row.id)} aria-expanded={open}>
            ...
            <div className="fsw-trust-row">
              <span>{flowHeadline(row)}</span>
              <b>{row.flowValidation === "live" ? (row.flowSignal === "outflow" ? "资金偏弱" : row.flowSignal === "inflow" ? "资金偏强" : row.flowSignal === "mixed" ? "资金与价格分化" : "资金中性") : sectorTrust(row)}</b>
            </div>
            <div className="fsw-trust-row"><span>{flowDetail(row)}</span><b>{row.turnover == null ? "" : `成交额 ${moneyCompact(row.turnover)}`}</b></div>
