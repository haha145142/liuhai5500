...
function readCache(): Cache | null { if (typeof window === "undefined") return null; try { const raw = localStorage.getItem(CACHE_KEY); if (!raw) return null; const v = JSON.parse(raw) as Cache; return v?.rows?.length ? v : null; } catch { return null; } }
...
function money(v: number | null) { if (v == null || !Number.isFinite(v)) return "—"; return `¥${v.toLocaleString("zh-CN", { maximumFractionDigits: 0 })}`; }
...
function sectorTrust(row: FundSectorQuote) { if (!row.validCount) return "暂无数据"; if (row.crossCheckedPct >= 80 && row.coveragePct >= 70) return "高可信"; if (row.crossCheckedPct >= 50 && row.coveragePct >= 50) return "中可信"; return "低可信"; }
...
export function FundSectorWatch() {
  ...
  const refresh = async (force = false) => {
    ...
    setLoading(true);
    try { const result = await getFundSectorQuotes({ data: { ids: validSelected } }); if (result.rows.some((r) => r.validCount > 0)) { setRows(result.rows); saveCache(result.rows, result.rows.find((r) => r.marketDate)?.marketDate || tradingDateLabel()); setStale(result.weekend); } else if (cached?.rows?.length) { setRows(cached.rows.filter((r) => validSelected.includes(r.id))); setStale(true); } else { setRows(result.rows); setStale(false); } }
  ...
  return <section className="fsw-wrap">
    ...
    <div className="fsw-list">{rows.length ? rows.map((row)=>{ const open=expanded===row.id; const valid=row.funds.filter((f)=>f.pct!=null); const leaders=valid.slice().sort((a,b)=>(b.pct??-Infinity)-(a.pct??-Infinity)); const displayFunds=open?leaders:valid; const heldFunds=row.funds.filter((f)=>holdingMap.has(f.code)); const heldValue=heldFunds.reduce((sum,f)=>{ const h=holdingMap.get(f.code); if(!h) return sum; const q=displayQuote(f); return sum+(q.value!=null?q.value*h.shares:h.cost*h.shares); },0); const impact=heldFunds.reduce((sum,f)=>{ const h=holdingMap.get(f.code); if(!h||f.pct==null) return sum; const q=displayQuote(f); const value=q.value!=null?q.value*h.shares:h.cost*h.shares; return sum+value*f.pct/100; },0); return <article key={row.id} className={`fsw-card ${open?"open":""}`}>
