...
export function OperationAdvice({ sectors, benchPct }:{ sectors:SectorQuote[]; benchPct:number|null }) {
  ...
  const rows = useMemo<BoardAdvice[]>(() => selectedCodes.map((code) => {
    ...
    const sector: SectorQuote = {
      ...
      change,
      flow: base?.flow ?? null,
      super: base?.super ?? null,
      large: base?.large ?? null,
      mid: base?.mid ?? null,
      small: base?.small ?? null,
      turnover: base?.turnover ?? quote.turnover ?? null,
      available: change != null,
      streak: base?.streak ?? 0,
      etfCode: base?.etfCode,
      etfName: base?.etfName,
      etfChange: base?.etfChange,
      validation: change == null ? "unavailable" : base?.validation === "cross_checked" ? "cross_checked" : quote.validation === "live" ? "single_source" : "unavailable",
    };
  ...
  useEffect(() => {
    ...
    const facts = rows.map(({ sector, factor }) => `${sector.name}：今日涨跌 ${sector.change == null ? "未知" : `${sector.change.toFixed(2)}%`}；板块主力净流入 ${sector.flow == null ? "未知" : `${(sector.flow / 1e8).toFixed(2)}亿`}；趋势 ${factor.trendLabel}；波段 ${factor.band}；规则建议 ${factor.advice}；置信 ${factor.confidence}%。`).join("\n");
    setAiLoading(true);
    void analyzeDeepSeek({ data: { apiKey: key, model: getDSModel(), prompt: `你是 Fund AI Pro 的解释层。只允许基于下面用户自选板块的今日涨跌、板块资金和规则结果回答。严禁创造数字、补猜资金、添加未选择板块。资金字段只有在输入中存在时才可引用；缺失就写暂无可靠数据。用中文输出3-5句，说明强弱排序以及当前更适合观察/持有/谨慎/规避，并明确证据来自今日板块涨跌、可用资金和规则结果。\n\n${facts}` } })
  ...
  const hasFlow = rows.some((x) => x.sector.flow != null);
  const flowText = rows.reduce((sum, x) => sum + (x.sector.flow ?? 0), 0);
  ...
  return <section className="mb-3 overflow-hidden rounded-[24px] border border-white/75 bg-white/48 p-3 shadow-[0_14px_38px_rgba(38,78,112,.07)] backdrop-blur-[20px] saturate-150">
    <div className="flex items-end justify-between gap-3"><div className="min-w-0"><div className="text-[16px] font-semibold tracking-tight text-fg">🎯 自选板块 · 操作建议 <span className="text-[10px] font-normal text-muted">仅供参考</span></div><div className="mt-0.5 text-[9px] text-muted">只读取你已添加板块的今日涨跌、可用资金与沪深300基准，不再用缺失数据硬凑结论</div></div><Link to="/ai" className="shrink-0 text-[9px] text-blue-600">AI复核 →</Link></div>
    {!selectedCodes.length ? <div className="mt-2.5 rounded-[16px] border border-dashed border-slate-200 bg-white/54 px-3 py-5 text-center text-[9px] text-slate-400">还没有选择板块。先在“自选板块”添加板块，这里只会显示你选择的板块。</div> : <>
      ...
      <div className="mt-2.5 grid grid-cols-2 gap-2"><div className="rounded-[16px] bg-white/58 p-2.5"><div className="text-[10px] font-semibold text-fg">🏗️ 强势观察</div><div className="mt-1 text-[9px] leading-[1.45] text-muted">{top}。以今日涨跌、可用资金和规则结果共同观察，不用不存在的数据补结论。</div></div><div className="rounded-[16px] bg-white/58 p-2.5"><div className="text-[10px] font-semibold text-fg">📊 今日多空</div><div className="mt-1 text-[9px] leading-[1.45] text-muted">{positive} 涨 / {negative} 跌{avg != null ? ` · 综合位置 ${avg.toFixed(0)}/100` : ""}{hasFlow ? ` · 自选板块主力净流入 ${flowText >= 0 ? "+" : ""}${(flowText / 1e8).toFixed(2)}亿` : " · 资金数据暂无可靠值"}。不自动交易。</div></div></div>
      <div className="mt-2.5 rounded-[16px] bg-blue-50/50 p-2.5 ring-1 ring-blue-100/60"><div className="flex items-center justify-between gap-2"><div className="text-[11px] font-semibold text-fg">🤖 DeepSeek 解读</div><span className="text-[8px] text-slate-400">{getDSKey() ? (aiLoading ? "分析中…" : hasFlow ? "基于涨跌 + 可用资金" : "基于今日涨跌") : "未配置 Key · 使用规则结果"}</span></div><div className="mt-1.5 text-[9px] leading-[1.6] text-muted">{aiText || "DeepSeek 未生成新的文字解读时，上面的规则结果仍然有效；不会用 AI 猜资金或数字。"}</div></div>
      <div className="mt-2.5 rounded-[16px] border border-amber-200/70 bg-amber-50/58 p-2.5"><div className="text-[11px] font-semibold text-amber-700">⚠️ 风险提示</div><div className="mt-0.5 text-[9px] leading-[1.5] text-muted">自选板块中 {sharpDown} 个单日跌幅超过 2%；今日统计 {positive} 涨 / {negative} 跌。没有可靠数据的字段会保持空白，不用模板数字填充。</div></div>
    </>}
  </section>;
}
