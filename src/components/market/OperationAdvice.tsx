...
export function OperationAdvice({ sectors, benchPct }:{ sectors:SectorQuote[]; benchPct:number|null }) {
  ...
  useEffect(() => {
    ...
    void getBoardWatchQuotes({ data: { codes: selectedCodes } })
      .then((res) => { if (alive) setQuotes(res.rows); })
  ...
  const rows = useMemo<BoardAdvice[]>(() => selectedCodes.map((code) => {
    ...
    if (!rule || !quote) return null;
    const base = sectors.find((s) => s.id === rule.id);
    const sector: SectorQuote = {
      id: rule.id,
      name: rule.name,
      bkCode: rule.bkCode,
      change: quote.pct ?? base?.change ?? null,
      flow: quote.mainFlow ?? base?.flow ?? null,
      super: quote.superFlow ?? base?.super ?? null,
      large: quote.largeFlow ?? base?.large ?? null,
      mid: quote.midFlow ?? base?.mid ?? null,
      small: quote.smallFlow ?? base?.small ?? null,
      turnover: quote.turnover ?? base?.turnover ?? null,
      available: quote.pct != null || base?.available === true,
      streak: base?.streak ?? 0,
      etfCode: base?.etfCode,
      etfName: base?.etfName,
      etfChange: base?.etfChange,
      validation: quote.validation === "live" ? "single_source" : "unavailable",
    };
    return { sector, factor: calcSixFactor(sector, benchPct), quote, source: quote.validation === "live" ? "东方财富实时资金流" : "暂无可靠资金流" };
  }).filter((x): x is BoardAdvice => x !== null), [benchPct, quotes, sectors, selectedCodes]);

  useEffect(() => {
    const key = getDSKey();
    if (!key || !rows.length) return;
    const facts = rows.map(({ sector, factor, source }) => `${sector.name}：涨跌 ${sector.change == null ? "未知" : `${sector.change.toFixed(2)}%`}；主力资金 ${flow(sector.flow)}；趋势 ${factor.trendLabel}；波段 ${factor.band}；规则建议 ${factor.advice}；置信 ${factor.confidence}%；${source}`).join("\n");
    setAiLoading(true);
    void analyzeDeepSeek({ data: { apiKey: key, model: getDSModel(), prompt: `你是 Fund AI Pro 的解释与评级层。只允许解释下面用户自选板块的真实数据和规则结果。严禁创造数字、补猜资金、添加未选择板块，也不要修改规则结果。用中文给出3-6句，说明资金与趋势是否一致、哪些更强或更弱、当前偏观察/持有/谨慎/规避。\n\n${facts}` } })
