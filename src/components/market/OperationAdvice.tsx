...
function matchAk(rows: AkRow[], rule: (typeof SECTOR_RULES)[number] | undefined) {
...
export function OperationAdvice({ sectors, benchPct }:{ sectors:SectorQuote[]; benchPct:number|null }) {
  ...
  const rows = useMemo<BoardAdvice[]>(() => selectedCodes.map((code) => {
    ...
    const ak = matchAk(rule.prefer === "industry" ? akIndustry : akConcept, rule);
    ...
    return { sector, factor: calcSixFactor(sector, benchPct), quote, source: ak ? "AKShare真实资金流" : quote.validation === "live" ? "东方财富实时资金流" : "暂无可靠资金流" };
  ...
  return <section className="mb-3 overflow-hidden rounded-[24px] border border-white/75 bg-white/48 p-3 shadow-[0_14px_38px_rgba(38,78,112,.07)] backdrop-blur-[20px] saturate-150">
    <div className="flex items-end justify-between gap-3"><div className="min-w-0"><div className="text-[16px] font-semibold tracking-tight text-fg">🎯 操作建议 <span className="text-[10px] font-normal text-muted">仅供参考</span></div><div className="mt-0.5 text-[9px] text-muted">只分析自选板块 · AKShare真实资金流 + 六因子规则 + DeepSeek解释</div></div><Link to="/ai" className="shrink-0 text-[9px] text-blue-600">AI复核 →</Link></div>
