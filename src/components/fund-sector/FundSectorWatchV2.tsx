...
export function FundSectorWatchV2(_props?: { portfolio?: Holding[]; funds?: Record<string, FundQuote> }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!alive) return;
        if (!result) { setMessage("实时板块行情响应较慢，稍后自动再试"); setLoading(false); return; }
        setQuotes(result.rows);
        setMessage(result.weekend ? "休市 · 沿用最近交易日板块数据" : "实时板块行情已更新");
        setLoading(false);
      } catch { if (alive) { setMessage("板块行情暂时不可用，不伪造涨跌数据"); setLoading(false); } }
  ...
  return (
    <section className="mb-3 overflow-hidden rounded-[24px] border border-white/70 bg-white/42 p-3 shadow-[0_18px_50px_rgba(38,78,112,.07),inset_0_1px_0_rgba(255,255,255,.94)] backdrop-blur-[24px] saturate-150">
      ...
      {searchOpen ? <div className="mt-3 rounded-[18px] border border-white/80 bg-white/75 p-2.5 shadow-[0_8px_24px_rgba(38,78,112,.05)]">
        <div className="flex items-center gap-2 rounded-[14px] border border-slate-200/75 bg-white px-3 py-2.5"><Search size={15} className="shrink-0 text-slate-400" /><input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="输入：半导体、机器人、银行、光伏……" className="min-w-0 flex-1 bg-transparent text-[13px] text-slate-900 outline-none placeholder:text-slate-400" />{searching ? <span className="text-[9px] text-slate-400">搜索中</span> : null}</div>
