...
export function FundSectorWatchV2() {
  ...
  useEffect(() => { if (!codeList.length) { setQuotes([]); setLoading(false); setMessage(""); return; } let alive = true; const run = async () => { setLoading(true); try { let result: { rows: BoardWatchQuote[]; weekend: boolean } | null = null; try { result = await Promise.race([getBoardWatchQuotes({ data: { codes: codeList } }), new Promise<null>((resolve) => window.setTimeout(() => resolve(null), 8000))]); } catch { result = null; } if (!alive) return; if (result?.rows?.length) { setQuotes(result.rows); setMessage(result.weekend ? "休市 · 沿用最近交易日板块数据" : "实时板块行情已更新"); } else { try { const direct = await fetchBoardQuotesJsonp(codeList); if (alive) { setQuotes(direct); setMessage(direct.length ? "已切换东方财富浏览器直连" : "板块暂无可靠数据"); } } catch { if (alive) setMessage("板块数据源暂时不可用，不伪造涨跌数据"); } } } finally { if (alive) setLoading(false); } }; void run(); const refresh = window.setInterval(run, BOARD_REFRESH_MS); return () => { alive = false; window.clearInterval(refresh); }; }, [codeList.join(",")]);
  ...
  const add = (item: BoardCandidate) => { setState((prev) => ({ items: [...prev.items, { code: item.code, name: item.name, icon: item.icon }] })); if (typeof window !== "undefined") window.dispatchEvent(new Event("fund-ai-pro-board-watch-change")); setSearchOpen(false); setQuery(""); setSuggestions([]); setOpenCode(item.code); setMessage(""); void loadSectorFunds(item.code); };
  ...
  const remove = (code: string) => { setState((prev) => ({ items: prev.items.filter((x) => x.code !== code) })); if (typeof window !== "undefined") window.dispatchEvent(new Event("fund-ai-pro-board-watch-change")); if (openCode === code) setOpenCode(null); };
