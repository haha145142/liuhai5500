...
export function FundSectorWatchV2(_props?: { portfolio?: Holding[]; funds?: Record<string, FundQuote> }) {
  ...
  useEffect(() => {
    if (!codeList.length) { setQuotes([]); setLoading(false); return; }
    let alive = true;
...
function FlowRow({ label, value }: { label: string; value: number | null }) { return <div className="flex items-center justify-between gap-2 rounded-xl bg-white/62 px-2.5 py-1.5"><span className="text-slate-500">{label}</span><span className={`font-semibold tabular-nums ${tone(value)}`}>{formatFlow(value)}</span></div>; }
