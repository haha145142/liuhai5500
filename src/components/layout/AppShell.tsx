...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { hydrate(); const a=window.setTimeout(refreshCore,80); const b=wantsNews?window.setTimeout(()=>void refreshNews(),450):undefined; return()=>{window.clearTimeout(a); if(b!==undefined)window.clearTimeout(b)}; }, [hydrate,refreshCore,refreshNews,wantsNews]);
  useEffect(() => { const id=window.setInterval(refreshCore,Math.max(180_000,settings.autoRefreshMs)); return()=>window.clearInterval(id); }, [refreshCore,settings.autoRefreshMs]);
