...
export function SmartAlerts() {
  ...
  const add = () => {
    ...
    const validTarget = kindNeedsFund ? /^\d{6}$/.test(code) : code.startsWith("sector:");
  ...
  return <Glass><SectionTitle title="智能提醒" hint="本机保存 · 30秒检查 · 1小时冷却" />
    <div className="rounded-2xl bg-bg-elevated px-3 py-3">
      ...
      <button type="button" onClick={add} disabled={kindNeedsFund ? !/^\d{6}$/.test(code) : !code.startsWith("sector:")} className="mt-2 h-10 w-full rounded-xl bg-accent text-xs font-semibold text-accent-fg disabled:opacity-50">添加提醒规则</button>
