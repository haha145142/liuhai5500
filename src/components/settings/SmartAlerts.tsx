  ...
  return <Glass><SectionTitle title="智能提醒" hint="本机保存 · 30秒检查 · 1小时冷却" />
    <div className="rounded-2xl bg-bg-elevated px-3 py-3">
      <div className="grid grid-cols-3 gap-1.5">
        <select value={code} onChange={(e) => setCode(e.target.value)} aria-label="提醒基金" className="h-10 rounded-xl bg-white/70 px-2 text-[10px] outline-none ring-1 ring-border"><option value="">选择基金</option>{codes.map((x) => <option key={x} value={x}>{x}</option>)}</select>
        <select value={kind} onChange={(e) => setKind(e.target.value as AlertKind)} aria-label="提醒类型" className="h-10 rounded-xl bg-white/70 px-2 text-[10px] outline-none ring-1 ring-border"><option value="fund_up">涨幅 ≥</option><option value="fund_down">跌幅 ≤</option><option value="drawdown">回撤 ≥</option></select>
        <input value={threshold} onChange={(e) => setThreshold(e.target.value)} inputMode="decimal" aria-label="提醒阈值" className="h-10 rounded-xl bg-white/70 px-2 text-[10px] outline-none ring-1 ring-border" placeholder="阈值 %" />
      </div>
    ...
    <p className="mt-2 text-[8px] leading-relaxed text-subtle">涨跌提醒使用当前基金日涨跌；回撤提醒使用当前缓存历史净值计算最大回撤。浏览器保持页面打开时可触发系统通知；没有可靠行情时不会凭空触发。</p>
  </Glass>;
}
