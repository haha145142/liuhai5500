...
export function FundCard({ holding, fund, sector, benchPct, onRemove, onUpdate }: {
  ...
  const quoteSourceLabel = officialToday ? "今日官方净值已发布，采用正式净值" : useEstimate ? `官方今日净值尚未发布，使用盘中估值${fund?.estimateConfidence ? `（${fund.estimateConfidence === "high" ? "高" : fund.estimateConfidence === "medium" ? "中" : "低"}置信度）` : ""}` : "非交易时段采用最近官方净值";
  return (
    <article className="glass mb-3 p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0"><div className="text-lg font-semibold text-fg">{name} <span className="text-xs font-normal text-muted">{holding.code}</span></div><div className="mt-1 text-xs text-muted">{quoteLabel} · {fund?.source || "等待数据"}</div></div>
        <div className="text-right"><Tone v={day} className="text-2xl font-semibold">{fmtPctShort(day)}</Tone><div className="text-[10px] text-muted">{officialToday ? "官方净值涨跌" : useEstimate ? "盘中实时估值涨跌" : "最近官方净值涨跌"}</div></div>
      </div>
      <div className="mt-4 rounded-2xl bg-white/60 p-3 ring-1 ring-white/70"><div className="grid grid-cols-2 gap-3"><div><div className="text-xs text-muted">当前价格</div><div className="mt-1 text-xl font-semibold tabular-nums">{fmtPrice(px, 4)}</div></div><div className="text-right"><div className="text-xs text-muted">持仓市值</div><div className="mt-1 text-xl font-semibold tabular-nums">{fmtMoney(value)}</div></div></div><div className="mt-2 text-[11px] text-muted">{fund?.nav != null ? `官方净值 ${fmtPrice(fund.nav, 4)} · ${fund.navDate || "日期未知"}` : "官方净值暂无"}{fund?.estimate != null ? ` · 盘中估值 ${fmtPrice(fund.estimate, 4)}${fund.estimateTime ? ` · ${fund.estimateTime}` : ""}` : ""}</div><div className="mt-1 text-[11px] font-semibold text-fg">口径：{quoteSourceLabel}</div>{estimateGap != null ? <div className="mt-1 text-[11px] text-muted">估值校验：相对最近官方净值 <Tone v={estimateGap}>{fmtPctShort(estimateGap)}</Tone></div> : null}</div>
