...
function BandPage() {
  ...
  if (!portfolio.length) return <Glass><EmptyNote>添加持仓后，这里会显示完整的波段信号、实时估值与做T计划。</EmptyNote></Glass>;
  return (
  ...
  return (
    <Glass className="mb-0 overflow-hidden rounded-[20px] border border-white/78 bg-white/52 p-2.5 shadow-[0_10px_28px_rgba(38,78,112,.06)] backdrop-blur-[18px] saturate-150">
      <div className="flex items-center justify-between gap-2">
        ...
        <Tone v={quote.pct} className="shrink-0 text-[15px] font-bold">{quote.pct == null ? "—" : fmtPctShort(quote.pct)}</Tone>
      </div>
...
function SwingAdviceCard({ swing, quote }: { swing: ReturnType<typeof calcSwingTrade>; quote: ReturnType<typeof selectFundDisplayQuote> }) {
  return <section className="mt-2.5 overflow-hidden rounded-[18px] border border-amber-200/70 bg-amber-50/45 p-2.5" aria-label="卖出提示 做T波段">
...
function SignalCard({ title, label, score, tone, subtitle }: { title:string; label:string; score:number|null; tone:"low"|"high"|"neutral"; subtitle:string }) { const pct = score == null ? 50 : Math.max(0, Math.min(100, score)); const labelClass = tone === "low" ? "text-[#66879e]" : tone === "high" ? "text-[#997d6d]" : "text-[#647588]"; return <div className="min-h-[108px] rounded-[15px] border border-white/80 bg-white/55 px-2.5 py-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,.9)]"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="text-[8px] font-medium text-muted">{title}</div><div className={`mt-0.5 text-[17px] font-bold leading-none ${labelClass}`}>{label}</div></div><div className="text-right"><div className="text-[7px] text-subtle">评分</div><div className={`text-[11px] font-bold tabular-nums ${labelClass}`}>{score == null ? "—" : `${score}/100`}</div></div></div><p className="mt-1.5 min-h-[25px] text-[8px] leading-[1.35] text-muted">{subtitle}</p><div className="mt-2 relative h-1.5 overflow-hidden rounded-full" style={{background:"linear-gradient(90deg,#e8c6c3 0%,#eee0c6 25%,#d5dce4 50%,#c3d0df 70%,#c7ddd8 100%)"}}><span className="absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 rounded-full border border-slate-400/70 bg-white/92 shadow-sm" style={{left:`calc(${pct}% - 7px)`}} /></div><div className="mt-0.5 flex justify-between text-[6.5px] text-subtle"><span>弱</span><span>中</span><span>强</span></div></div>; }
