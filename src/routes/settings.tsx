    ...
    <div className="space-y-3">
      <Glass tight className="overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-3 rounded-2xl bg-bg-elevated/70 px-3 py-3">
          <div><div className="text-xs font-semibold text-fg">当前市场数据状态</div><div className="mt-1 text-[10px] text-subtle">所有页面统一使用同一套交易时段规则</div></div>
      ...
      <Glass>
        ...
        {sources.length ? <div className="space-y-1.5">{sources.map((s, i) => <div key={`${s.name}-${i}`} className="flex items-center gap-3 rounded-2xl bg-bg-elevated px-3 py-2.5"><span className={`size-2 shrink-0 rounded-full ${s.status === "ok" ? "bg-emerald-500" : "bg-slate-400"}`} /><div className="min-w-0 flex-1"><div className="text-xs font-semibold text-fg">{s.name}</div><div className="mt-0.5 truncate text-[10px] text-subtle">{s.note}</div></div><span className={`text-[10px] font-semibold ${s.status === "ok" ? "tone-down" : "text-muted"}`}>{s.status === "ok" ? "正常" : "暂不可用"}</span></div>)}</div> : <p className="text-sm text-muted">尚未取得数据源状态。</p>}
      ...
      <Glass>
        ...
        <div className="rounded-2xl bg-bg-elevated px-3 py-3">
          <div className="flex items-center justify-between"><label className="text-xs font-semibold text-fg">行情自动刷新（秒）</label><span className="text-xs font-bold text-accent tabular-nums">{refreshSeconds}s</span></div>
      ...
      <Glass>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={save} className="h-11 rounded-2xl bg-fg text-sm font-semibold text-bg">保存</button><button type="button" onClick={() => void test()} disabled={testing} className="h-11 rounded-2xl bg-accent text-sm font-semibold text-accent-fg disabled:opacity-60">{testing ? "测试中…" : "测试连接"}</button></div>
