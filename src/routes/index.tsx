...
function Home() {
  ...
  const statusText = isWeekend()
    ...
    : (missingCount ? `${missingCount} 项指数暂缺可靠值` : "指数数据正常");
  ...
  return (
    <div className="home-page">
        ...
        <Glass tight className="home-hero mb-3">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-1 home-value tabular-nums">{fmtMoney(analysis.marketValue)}</div>
              <div className="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-1">
                <Tone v={analysis.dayPnl} className="home-day-pnl font-bold tabular-nums">
                  {analysis.dayPnl == null ? "今日盈亏暂无可靠数据" : `${analysis.dayPnl >= 0 ? "今日盈利 " : "今日亏损 "}${fmtMoney(Math.abs(analysis.dayPnl))}`}
                </Tone>
                {analysis.dayPct != null ? <span className="home-day-pct tabular-nums">{fmtPctShort(analysis.dayPct)}</span> : null}
              </div>
            </div>
          ...
          <div className="mt-3 flex items-center justify-between gap-3 home-meta-row">
            <span>累计收益 {analysis.pnlPct == null ? "暂无可靠数据" : fmtPctShort(analysis.pnlPct)}</span>
            <span>覆盖 {analysis.holdingsCovered}/{analysis.holdingsTotal}</span>
          </div>
        </Glass>
      ) : (
        <Glass tight className="home-hero mb-3">
        ...
        <Glass tight className="home-market-strip mb-3">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-0.5 truncate text-sm font-semibold">{isWeekend() ? "休市 · 最近交易日" : "交易日 · 后台更新"}</div>
            </div>
      ...
      <div className="home-section-heading">
        <div><div className="text-base font-semibold tracking-tight text-fg">大盘</div><div className="text-[10px] text-subtle">最近可验证行情</div></div>
