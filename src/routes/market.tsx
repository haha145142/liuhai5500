...
import { EmptyNote, Tone } from "@/components/ui/Glass";
...
export const Route = createFileRoute("/market")({
  loader: async () => {
    const [snapshotResult, fallbackResult, globalResult] = await Promise.allSettled([getSnapshot(), getLatestTradingMarketData(), getGlobalMarketFallback()]);
    ...
    return { ...base, indices, sectors, global: [...global, ...extraGlobal], marketDate: base.marketDate || fallback?.marketDate || null } satisfies Snapshot;
  },
...
function MarketPage() {
  ...
  const [open, setOpen] = useState<Record<FoldKey, boolean>>({ advice: true, valuation: true, money: true, global: true });
  const toggle = (key: FoldKey) => setOpen((prev) => ({ ...prev, [key]: !prev[key] }));

  return <div className="market-page">
    <FundSectorWatchV2 />
    <FoldSection title="操作建议" subtitle={benchPct == null ? "根据最近交易日与自选板块数据生成" : `大盘 ${fmtPctShort(benchPct)} · 根据可验证数据生成`} open={open.advice} onToggle={() => toggle("advice")}><OperationAdvice sectors={snapshot?.sectors || []} benchPct={benchPct} /></FoldSection>
    {snapshot?.indices?.length ? <IndexGrid indices={snapshot.indices} /> : <EmptyNote>正在读取最近交易日行情…</EmptyNote>}
    ...
    <FoldSection title="资金雷达" subtitle={snapshot?.flow ? `主力净流入 ${formatMoney(snapshot.flow.main)} · 资金与行情` : "资金行情"} open={open.money} onToggle={() => toggle("money")}><MarketPanorama /></FoldSection>
    <FoldSection title="外围市场" subtitle={globalSummary(snapshot?.global)} open={open.global} onToggle={() => toggle("global")}>
      <div className="rounded-[18px] bg-white/52 p-2 ring-1 ring-white/70">
        <div className="grid grid-cols-2 gap-2">{(snapshot?.global ?? []).map((g) => <div key={g.name} className="rounded-2xl bg-bg-elevated p-3"><div className="text-xs text-muted">{g.name}</div><div className="mt-1 text-sm font-semibold tabular-nums text-fg">{g.price == null ? "—" : g.price.toFixed(g.price >= 1000 ? 2 : 4)}</div><Tone v={g.pct} className="mt-0.5 text-base font-semibold">{g.pct == null ? "—" : fmtPctShort(g.pct)}</Tone></div>)}</div>
      </div>
    </FoldSection>
...
function FoldSection({ title, subtitle, open, onToggle, children }: { title: string; subtitle: string; open: boolean; onToggle: () => void; children: ReactNode }) {
  return <section className="mt-3 overflow-hidden rounded-[24px] border border-white/75 bg-white/44 shadow-[0_16px_44px_rgba(38,78,112,.055),inset_0_1px_0_rgba(255,255,255,.95)] backdrop-blur-[22px] saturate-150">
    <button type="button" onClick={onToggle} className="flex w-full items-center gap-3 px-3.5 py-3 text-left active:bg-white/40" aria-expanded={open}>
      <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h2 className="text-[15px] font-semibold tracking-tight text-fg">{title}</h2></div><div className="mt-0.5 truncate text-[9px] text-subtle">{subtitle}</div></div>
