  ...
  return (
    <div>
      <Glass className="portfolio-shell mb-3 overflow-hidden rounded-[28px] border border-white/75 bg-white/48 p-3 shadow-[0_18px_48px_rgba(38,78,112,.07),inset_0_1px_0_rgba(255,255,255,.95)] backdrop-blur-[20px] saturate-150">
        <div className="flex items-center justify-between gap-2"><div className="min-w-0"><div className="text-[18px] font-bold tracking-tight text-fg">💼 我的持仓 <span className="ml-1 text-[10px] font-normal text-muted">盘中估值 · 收盘以官方净值为准</span></div></div><span className="shrink-0 rounded-full bg-blue-100/75 px-2.5 py-1 text-[11px] font-semibold text-blue-600">{portfolio.length}只</span></div>
        <div className="mt-2.5 flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[12px] text-muted">持仓总收益</div><Tone v={summary.holdingPnl} className="mt-1 block text-[32px] font-bold leading-none tracking-tight">{summary.pricedCount === summary.totalCount && summary.totalCount > 0 ? fmtMoney(summary.holdingPnl) : "—"}</Tone><Tone v={summary.holdingPnlPct} className="mt-1.5 block text-[16px] font-semibold">{summary.pricedCount === summary.totalCount && summary.holdingPnlPct != null ? fmtPctShort(summary.holdingPnlPct) : "—"}</Tone></div><div className="shrink-0 pt-4 text-right text-[10px] leading-relaxed text-muted"><div>今日 {downCount} 跌 / {upCount} 涨 / {Math.max(0, portfolio.length - downCount - upCount)} 平</div><div>成本 {fmtMoney(summary.costValue)}</div><div>持仓市值 {fmtMoney(summary.marketValue)}</div></div></div>
        <div className="mt-3 grid grid-cols-5 gap-1 rounded-2xl bg-white/48 p-0.5 ring-1 ring-white/75">{(["today","week","month","quarter","since"] as const).map((tab) => { const label = tab === "today" ? "今天" : tab === "week" ? "一周" : tab === "month" ? "一个月" : tab === "quarter" ? "近3月" : "买入以来"; return <button key={tab} type="button" onClick={() => setSummaryTab(tab)} className={`rounded-[14px] px-1.5 py-2 text-[12px] font-medium transition ${summaryTab === tab ? "bg-blue-500 text-white shadow-[0_4px_12px_rgba(59,130,246,.20)]" : "bg-white/66 text-muted"}`}>{label}</button>; })}</div>
        <div className="mt-2.5 rounded-2xl bg-white/54 px-3 py-2 ring-1 ring-white/70"><div className="flex items-center justify-between gap-3"><span className="text-[12px] text-muted">{selectedSummary.label}</span><Tone v={selectedSummary.amount} className="text-[18px] font-bold">{selectedSummary.amount == null ? "—" : fmtMoney(selectedSummary.amount)}</Tone></div><div className="mt-0.5 text-right"><Tone v={selectedSummary.pct} className="text-[12px] font-semibold">{selectedSummary.pct == null ? "—" : fmtPctShort(selectedSummary.pct)}</Tone></div></div>
        {summary.totalCount > summary.pricedCount ? <div className="mt-1.5 rounded-xl bg-amber-50/70 px-3 py-1.5 text-[9px] text-muted">还有 {summary.totalCount - summary.pricedCount} 只基金暂未取得可靠行情；无法确认的数字不猜。</div> : null}
        {summaryTab !== "today" && summaryTab !== "since" && selectedSummary.amount == null ? <div className="mt-1.5 rounded-xl bg-amber-50/70 px-3 py-1.5 text-[9px] text-muted">该周期需要全部持仓具备对应历史净值；数据不完整时不显示估算收益。</div> : null}
        <div className="mt-2.5 space-y-2">{portfolio.length ? portfolio.map((h) => { const fname = funds[h.code]?.name || h.name; const rule = matchFundSector(fname); const sector = rule ? snapshot?.sectors.find((s) => s.id === rule.id) : undefined; return <FundCard key={h.code} holding={h} fund={funds[h.code]} sector={sector} benchPct={bench} totalMarketValue={summary.marketValue} onUpdate={(patch) => updateHolding(h.code, patch)} onRemove={() => removeHolding(h.code)} />; }) : <EmptyNote>还没有持仓。添加基金后会自动拉取官方净值、盘中估值和历史指标。</EmptyNote>}</div>
        <QuickAddFund />
      </Glass>
