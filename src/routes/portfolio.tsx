  ...
  return (
    <div>
      <Glass className="portfolio-shell mb-3 overflow-hidden rounded-[28px] border border-white/75 bg-white/48 p-3 shadow-[0_18px_48px_rgba(38,78,112,.07),inset_0_1px_0_rgba(255,255,255,.95)] backdrop-blur-[20px] saturate-150">
        ...
        <div className="mt-2.5 flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[12px] text-muted">整体盈亏</div><Tone v={selectedSummary.amount} className="mt-1 block text-[32px] font-bold leading-none tracking-tight">{selectedSummary.amount == null ? "—" : fmtMoney(selectedSummary.amount)}</Tone><Tone v={selectedSummary.pct} className="mt-1.5 block text-[16px] font-semibold">{selectedSummary.pct == null ? "—" : fmtPctShort(selectedSummary.pct)}</Tone></div><div className="shrink-0 pt-4 text-right text-[10px] leading-relaxed text-muted"><div>今日 {downCount} 跌 / {portfolio.length - downCount} 涨平</div><div>成本 {fmtMoney(summary.costValue)}</div><div>持仓市值 {fmtMoney(summary.marketValue)}</div></div></div>
        <div className="mt-3 grid grid-cols-5 gap-1 rounded-2xl bg-white/48 p-0.5 ring-1 ring-white/75">{(["today","week","month","quarter","since"] as const).map((tab) => { const label = tab === "today" ? "今天" : tab === "week" ? "本周" : tab === "month" ? "本月" : tab === "quarter" ? "近3月" : "买入以来"; return <button key={tab} type="button" onClick={() => setSummaryTab(tab)} className={`rounded-[14px] px-1.5 py-2 text-[12px] font-medium transition ${summaryTab === tab ? "bg-blue-500 text-white shadow-[0_4px_12px_rgba(59,130,246,.20)]" : "bg-white/66 text-muted"}`}>{label}</button>; })}</div>
