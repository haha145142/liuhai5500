...
function PortfolioPage() {
  ...
  const pricedToday = portfolio.map((h) => holdingReturns.get(h.code)).filter((r): r is NonNullable<typeof r> => r?.todayPnlPct != null);
  const downCount = pricedToday.filter((r) => r.todayPnlPct! < 0).length;
  const upCount = pricedToday.filter((r) => r.todayPnlPct! > 0).length;
  const flatCount = pricedToday.filter((r) => r.todayPnlPct === 0).length;
  const unknownTodayCount = Math.max(0, portfolio.length - pricedToday.length);
